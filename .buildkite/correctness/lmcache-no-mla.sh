...
export PORT=8000
export VLLM_MLA_DISABLE=1 # disable the MLA optimization of deepseek (compressing KV cache into latent space)
...
python3 .buildkite/correctness/mmlu_bench.py \
  --nsub 6 \
  --parallel 16 \
  > mmlu-results/lmcache_no_mla.txt || true

# Verify result file was created
if [ -f "mmlu-results/lmcache_no_mla.txt" ]; then
...
echo "✅ LMCache without MLA test completed"
