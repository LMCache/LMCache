...
def parse_result_file(filepath):
    ...
    try:
        ...
        for line in content.splitlines():
            ...
            elif match := re.match(r"Average accuracy ([0-9.]+), latency ([0-9.]+), #q: ([0-9]+) - (.+)", line):
...
def create_comparison_plots(all_results):
    ...
    ax3.scatter(latencies, accuracies, s=100, alpha=0.7,
               c=range(len(configs)), cmap='husl')
    ...
    for i, config in enumerate(configs):
        ax3.annotate(config, (latencies[i], accuracies[i]),
                    xytext=(5, 5), textcoords='offset points', fontsize=9)

    # 4. Performance efficiency (accuracy/latency ratio)
    efficiency = [acc/lat for acc, lat in zip(accuracies, latencies)]
...
def create_subject_breakdown(all_results):
    ...
    bars = ax.bar(range(len(subjects)), accuracies, color=sns.color_palette("viridis", len(subjects)))

    ax.set_title(f'Subject-wise Accuracy Breakdown ({best_result["config"]})', fontweight='bold')
...
def create_summary_table(all_results):
    ...
    headers = ['Configuration', 'Accuracy', 'Latency', 'Total Questions', 'Subjects Tested']
    ...
    ax.set_title('MMLU Benchmark Results Summary', fontweight='bold', fontsize=16, pad=20)
...
def generate_report():
    ...
    for filepath in result_files:
        ...
        print(f"📊 Parsed {result['config']}: acc={result['accuracy']}, lat={result['latency']}")
