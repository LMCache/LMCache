...
#              | mp_autostart_tp2 | restart_recovery | gds_smoke_test | p2p
#              | kimi_linear_tp | dsv4_flash_tp | lazy_offload
#              | hybrid_lazy_offload
# Thin wrapper: sets up environment, then delegates to scripts/.
# No Docker -- all processes run natively in the pod.
set -euo pipefail

TEST_NAME="${1:?Usage: $0 <test_name>  (lm_eval|lm_eval_preemption|hma_lm_eval_gemma4|hma_lm_eval_qwen3_5|hybrid_lazy_offload|vllm_bench|long_doc_qa|long_doc_qa_l2|fault_tolerance|deadlock|mp_autostart_tp2|restart_recovery|cache_stats|lazy_offload|http_api|p2p|kimi_linear_tp|dsv4_flash_tp)}"
