...
#   test_name: lm_eval | hma_lm_eval | hma_lm_eval_gemma4 | hma_lm_eval_gemma4_e4b
#              | vllm_bench | long_doc_qa | long_doc_qa_l2 | fault_tolerance
#              | deadlock | restart_recovery
# Thin wrapper: sets up environment, then delegates to scripts/.
# No Docker -- all processes run natively in the pod.
set -euo pipefail

TEST_NAME="${1:?Usage: $0 <test_name>  (lm_eval|hma_lm_eval|hma_lm_eval_gemma4|hma_lm_eval_gemma4_e4b|vllm_bench|long_doc_qa|long_doc_qa_l2|fault_tolerance|deadlock|restart_recovery|cache_stats|http_api)}"
