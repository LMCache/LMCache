...
elif [ "$TEST_NAME" = "deepseek_v4_flash_tp" ]; then
...
SELF_CONTAINED_TESTS=" deadlock p2p kimi_linear_tp deepseek_v4_flash_tp "
...
case "$TEST_NAME" in
    ...
    kimi_linear_tp)
        ...
        ;;
    deepseek_v4_flash_tp)
        exec_script="${SCRIPT_DIR}/run-deepseek-v4-flash-tp.sh"
        ;;
    http_api)
    ...
    *)
        ...
        echo "Valid tests: lm_eval, lm_eval_preemption, hma_lm_eval_gemma4, hma_lm_eval_gemma4_engine_driven, vllm_bench, long_doc_qa, long_doc_qa_l2, fault_tolerance, deadlock, restart_recovery, cache_stats, http_api, gds_smoke_test, p2p, kimi_linear_tp, deepseek_v4_flash_tp"
        exit 1
        ;;
esac
