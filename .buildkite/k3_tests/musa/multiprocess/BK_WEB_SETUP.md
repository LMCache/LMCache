...
All MUSA CI files are organized under the `unittests/`, `e2e/`, and
`multiprocess/` directories:

| Path | Role |
|------|------|
| `unittests/` | Broad unit suite |
| `multiprocess/` | Transfer tests and MP server smoke |
| `e2e/` | vLLM and SGLang serving correctness |
...
|---------------|---------------------|---------------------|
| `unit-tests-musa` | `unittests/buildkite-pipeline.yml` | `unittests/pipeline.yml` |
| `musa-mp-test` | `multiprocess/buildkite-pipeline.yml` | `multiprocess/pipeline.yml` |
...
different device, change this value in `unittests/pipeline.yml` and
`multiprocess/pipeline.yml` to that explicit device index. Do not expose
more than one GPU to either lane.
...
steps:
  - label: ":pipeline: Upload pipeline"
    command: bash .buildkite/k3_tests/common_scripts/upload-pipeline.sh .buildkite/k3_tests/musa/unittests/pipeline.yml
```
...
steps:
  - label: ":pipeline: Upload pipeline"
    command: bash .buildkite/k3_tests/common_scripts/upload-pipeline.sh .buildkite/k3_tests/musa/multiprocess/pipeline.yml
```
