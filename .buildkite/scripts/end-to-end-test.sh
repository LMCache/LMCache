...
LMCACHE_TRACK_USAGE="false" python3 main.py tests/tests.py \
  -f test_local_cpu_experimental -o outputs/ -p $port1 $port2 &
echo $! > .buildkite/bare_cpu_pid
wait $?
...
LMCACHE_TRACK_USAGE="false" python3 main.py tests/tests.py \
  -f test_local_disk_experimental -o outputs/ -p $port1 $port2 &
echo $! > .buildkite/bare_disk_pid
wait $?
