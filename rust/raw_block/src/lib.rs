...
//! Provides direct block device access with O_DIRECT support and async I/O scheduling.
...
/// Error information propagated from worker threads to completion thread.
#[derive(Clone, Debug)]
enum ErrorInfo {
...
/// Select FD from pool based on current CPU (per-CPU I/O for parallelism).
...
/// I/O task priority (higher priority processed first).
#[derive(Clone, Copy)]
enum Priority {
    High,    // Prefetch/read
    Medium,  // Normal ops
    Low,     // Background writes
}

/// Type of I/O operation.
enum TaskKind {
    Pwrite { offset: u64, payload_len: usize, total_len: usize },
    Pread { offset: u64, payload_len: usize, total_len: usize },
}

/// Python buffer held across threads (released under GIL in completion thread).
struct HeldPyBuffer(pyo3::ffi::Py_buffer);

// SAFETY: Py_buffer contains only pointers/integers. Buffer protocol keeps object alive.
...
/// Multi-FD pool for parallel I/O (one FD per CPU).
#[pyclass]
...
/// Async I/O scheduler with worker threads and priority queues.
#[pyclass]
