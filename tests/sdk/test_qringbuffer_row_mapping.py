...
class _Request:
    ...
    def row_slots(self) -> list[int]:
        ...
        for i in range(self.num_scheduled):
            ...
            slots.append(
                self.gpu_blocks[t // BLOCK_SIZE] * BLOCK_SIZE + t % BLOCK_SIZE
            )
        return slots
...
def _build_step(requests: list[_Request], row_order: list[int] | None = None):
    ...
    for idx in order:
        ...
        for i in range(req.num_scheduled):
            row_tags.append(float(hash((req.request_id, req.computed_offset + i)) % 100003))
    ...
    attn_metadata = SimpleNamespace(slot_mapping=torch.tensor(row_slots, dtype=torch.int64))
...
def _captured_tokens(capture, state, query, req: _Request) -> torch.Tensor | None:
    ...
    store = next(
        (s for s in state.stores if s.request_id == req.request_id), None
    )
    if store is None:
        return None
...
def test_missing_slot_mapping_disables_step():
    ...
    assert (
        capture._build_q_step_state(query, md, SimpleNamespace(foo=1)) is None
    )
