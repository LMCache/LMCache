...
def test_unregister_one_shared_gpu_layout_keeps_registry_until_last_instance(
    ...
    from lmcache.v1.multiprocess.modules import (
        engine_driven_transfer as engine_driven_transfer_mod,
    )
    ...
    monkeypatch.setattr(
        engine_driven_transfer_mod.torch_dev,
        "empty_cache",
        lambda: None,
        raising=False,
    )

    module = engine_driven_transfer_mod.EngineDrivenTransferModule(ctx)
