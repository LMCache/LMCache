...
def _make_engine():
    """A BlendService mock with only the attributes these paths touch."""
...
def _bind(eng, name):
    """Bind a real BlendService method to the mock engine."""
    return getattr(blend_mod.BlendService, name).__get__(eng)
...
class TestFingerprintJobTuple:
    ...
    def test_store_enqueues_request_id(self):
        ...
        eng._transfer_service.store.return_value = (b"handle", True)
...
class TestRetrieveEventsCarryWorkerId:
    ...
    def test_every_retrieve_and_scatter_publish_stamps_worker_id(self):
        ...
        # scatter: keep this in step with BlendService.cb_retrieve_pre_computed.
