...
def _make_engine_with_mocked_matcher():
    """Construct a real BlendService with the matcher mocked so we can
    ...
    eng_mock = MagicMock(spec=blend_mod.BlendService)
    ...
    eng_mock._emit_fingerprints_registered = (
        blend_mod.BlendService._emit_fingerprints_registered.__get__(eng_mock)
    )
    eng_mock._drain_fingerprint_queue = (
        blend_mod.BlendService._drain_fingerprint_queue.__get__(eng_mock)
    )
    return eng_mock
...
def _release_applied(matches, applied, n_read, stream):
    ...
    with patch.object(blend_mod, "submit_callback_to_stream") as cb:
        n = blend_mod.BlendService._release_applied_read_locks(
...
def test_batched_rope_calls_kernel_per_group_per_slot():
    ...
    eng = MagicMock(spec=blend_mod.BlendService)
    eng._apply_cb_rope_batched = blend_mod.BlendService._apply_cb_rope_batched.__get__(
        eng
    )
...
def _coord_engine(chunk_size: int = 4):
    """A BlendService mock with the coordinator-leg methods bound."""
    # First Party
    from lmcache.v1.multiprocess.services import blend as blend_mod
    ...
    eng._build_global_segments = blend_mod.BlendService._build_global_segments.__get__(
        eng
    )
    eng._poll_coordinator_match = (
        blend_mod.BlendService._poll_coordinator_match.__get__(eng)
    )
    return eng
...
def test_non_overlapping_after_prefix():
    ...
    f = blend_mod.BlendService._non_overlapping_after_prefix
...
def _rope_registration_engine(engine_group_indices: list[int]):
    """A BlendService mock with ``cb_register_rope`` bound and a registered
    ...
    eng = MagicMock(spec=blend_mod.BlendService)
    eng._cb_rope_state = {}
    eng._transfer_service = MagicMock()
    ...
    eng._transfer_service.get_and_touch_context_entry.return_value = entry
    eng.cb_register_rope = blend_mod.BlendService.cb_register_rope.__get__(eng)
    return eng
...
def test_register_rope_requires_registered_instance():
    ...
    eng._transfer_service.get_and_touch_context_entry.return_value = None
...
def _build_scatter_engine_and_context(
    ...
    eng._scatter_batch_to_paged = (
        blend_mod.BlendService._scatter_batch_to_paged.__get__(eng)
    )
...
def _build_plan_engine_and_context(
    ...
    for name in (
        ...
        setattr(eng, name, getattr(blend_mod.BlendService, name).__get__(eng))
...
def test_union_of_local_and_fleet_matches_collapses_duplicates():
    ...
    dedup = blend_mod.BlendService._non_overlapping_after_prefix
