...
def test_keys():
    ...
    return [IPCCacheEngineHashKey.from_int_hash("model1", 1, 0, i) for i in range(10)]
...
class TestThreadSafety:
    def test_concurrent_reserves(self, storage_manager, test_dtype, test_format):
        ...
        def reserve_keys(thread_id):
            keys = [
                IPCCacheEngineHashKey.from_int_hash("model1", 1, 0, thread_id * 100 + i)
    ...
    def test_concurrent_handle_allocation(
        ...
        def get_handle(thread_id):
            keys = [IPCCacheEngineHashKey.from_int_hash("model1", 1, 0, thread_id)]
            handle, _ = storage_manager.reserve(keys, shape, test_dtype, test_format)
            return handle
