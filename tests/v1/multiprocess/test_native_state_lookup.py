...
def test_endpoint_limit_requires_truncating_lookup_tokens() -> None:
    # Setting only key.end does not limit lookup: it hashes token_ids in full.
    full_hit, _storage, _module, _key_value = _lookup([[2, 5]], query_end=4 * CHUNK)
    assert full_hit == 5
    # Post-hoc min(full_hit, 4) is unsafe: there is no checkpoint at chunk 4.
    assert min(full_hit, 4) == 4
    limited_hit, storage, module, key = _lookup(
        [[2, 5]], query_tokens=TOKENS[: 4 * CHUNK]
    )
    assert limited_hit == 2
