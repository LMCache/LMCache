...
"""Blend matcher tests: optimized vs current lookup equivalence, and
content-fingerprint dedup (``--enable-dedup-content``).

Merged from test_optimized_lookup.py and test_blend_dedup_content.py in
the blend package split.
...
def _content_chunk(seed: int) -> list[int]:
    """Deterministic ``CHUNK_SIZE`` tokens derived from seed; unique per seed."""
    return [seed * CHUNK_SIZE + i + 1 for i in range(CHUNK_SIZE)]
...
def test_distinct_content_still_registers_when_enabled():
    ...
    assert _matched_hashes(
        matcher, _content_chunk(1) + _content_chunk(2) + _content_chunk(3)
    ) == {
...
def test_duplicate_content_within_one_batch_is_skipped():
    ...
    _register_content(matcher, _content_chunk(1) + _content_chunk(1), [101, 102])
...
def test_repeated_registration_of_same_hash_is_idempotent():
    ...
    matcher = BlendTokenRangeMatcher(chunk_size=CHUNK_SIZE, dedup_content=True)
    _register_content(matcher, _content_chunk(1), [101])
    _register_content(matcher, _content_chunk(1), [101], position_offset=CHUNK_SIZE)
