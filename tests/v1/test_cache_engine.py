...
def test_local_disk_persists_across_engine_restart(autorelease_v1):
    device = "cuda"
    num_tokens = 2000
    num_blocks = 1000
    block_size = 16
    ...
    chunk_size = 256
    kv_shape = (32, 2, chunk_size, 8, 128)
    instance_id = "test_local_disk_restart"
    ...
    expected_length = (len(tokens) // chunk_size) * chunk_size
    ...
    with tempfile.TemporaryDirectory(
        ...
        engine1 = autorelease_v1(
            LMCacheEngineBuilder.get_or_create(
                instance_id,
                cfg,
                dumb_metadata(kv_shape),
                create_gpu_connector(1024, 32),
                mock_up_broadcast_fn,
                mock_up_broadcast_object_fn,
            )
        )
        engine1.store(tokens=tokens, kvcaches=kv_cache, slot_mapping=slot_mapping)
        recover_engine_states(engine1)

        start_time = time.time()
        while (
            engine1.lookup(tokens, search_range=["LocalDiskBackend"]) < expected_length
        ):
            if time.time() - start_time > 30:
                raise TimeoutError("Disk store did not finish before restart test.")
            time.sleep(0.01)

        LMCacheEngineBuilder.destroy(instance_id)

        engine2 = autorelease_v1(
            LMCacheEngineBuilder.get_or_create(
                instance_id,
                cfg,
                dumb_metadata(kv_shape),
        ...
        assert (
            engine2.lookup(tokens, search_range=["LocalDiskBackend"]) == expected_length
        )

        ret_mask = engine2.retrieve(
            tokens,
            kvcaches=retrieved_cache,
            slot_mapping=slot_mapping,
        )
        recover_engine_states(engine2)

        assert torch.sum(ret_mask) == expected_length
        check_paged_kv_cache_equal(
            retrieved_cache,
            kv_cache,
            slot_mapping[:expected_length],
