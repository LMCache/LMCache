...
def test_same_retrieve_store(autorelease_v1):
    device = "cuda"
    fmt = "vllm"
    num_tokens = 2000
    chunk_size = 256
...
def test_paged_retrieve_prefix(
    ...
    connector = create_gpu_connector(
        1024,
        32,
        paged=True,
        use_list=use_list,
...
@pytest.mark.parametrize("fmt", ["vllm"])
def test_store_kv_tensors_mask(fmt, autorelease_v1):
    device = "cuda"
    num_tokens = 1000
    new_num_tokens = 2000
    chunk_size = 256
...
def test_prefetch_retrieve(backend, prefetch_from, autorelease_v1):
    device = "cuda"
    num_tokens = 2000
    new_num_tokens = 1000
    chunk_size = 256
