...
"""Integration tests for the Dynamo KV wiring (store + evict subscribers).

These exercise the bypass paths end to end without a GPU or a real server:
synthetic ``MP_KEYS_STORED`` events feed the store subscriber, and a real
EventBus drives the evict subscriber. The transport is a real ZMQ PUB bound to
a test address with a SUB peer asserting the received events.
"""
...
from lmcache.v1.mp_observability.dynamo.store_subscriber import (  # noqa: E402
...
def _store_event(
    token_ids: list[int], start: int, end: int, chunk_hashes: list[bytes]
) -> Event:
    """Build the ``MP_KEYS_STORED`` event the store subscriber consumes."""
    return Event(
        event_type=EventType.MP_KEYS_STORED,
        session_id="req-1",
        metadata={
            "key": _make_key(token_ids, start, end),
            "object_keys": _make_object_keys(chunk_hashes),
        },
    )
...
def test_store_subscriber_publishes_block_stored(
    ...
    # A synthetic MP_KEYS_STORED event drives the subscriber on purpose.
    subscriber = DynamoStoreSubscriber(publisher)
    subscriber._on_store(_store_event(token_ids, 0, 2 * KV_BLOCK_SIZE, [b"c0", b"c1"]))
...
def test_evict_via_event_bus_publishes_block_removed(
    ...
    DynamoStoreSubscriber(publisher)._on_store(
        _store_event(token_ids, 0, 2 * KV_BLOCK_SIZE, chunk_hashes)
    )
...
def test_store_subscriber_swallows_publisher_errors(
    wiring: tuple[DynamoKvPublisher, zmq.Socket, TokenHasher],
) -> None:
    ...
    # Must not raise -- the EventBus drain loop is unaffected by a failure.
    subscriber = DynamoStoreSubscriber(_BoomPublisher())  # type: ignore[arg-type]
    subscriber._on_store(_store_event([0, 1, 2], 0, 3, [b"c0"]))
    # And nothing was emitted on the wire.
    assert not sub.poll(300)


def test_store_via_event_bus_publishes_block_stored(
    wiring: tuple[DynamoKvPublisher, zmq.Socket, TokenHasher],
) -> None:
    publisher, sub, hasher = wiring
    token_ids = list(range(2 * KV_BLOCK_SIZE))

    # Drive the store through a real EventBus + subscriber (mirrors the evict
    # bus test): the registered subscriber turns MP_KEYS_STORED into BlockStored.
    bus = EventBus(EventBusConfig(enabled=True))
    bus.register_subscriber(DynamoStoreSubscriber(publisher))
    bus.start()
    try:
        bus.publish(_store_event(token_ids, 0, 2 * KV_BLOCK_SIZE, [b"c0", b"c1"]))
        batch = _recv_batch(sub)
    finally:
        bus.stop()
