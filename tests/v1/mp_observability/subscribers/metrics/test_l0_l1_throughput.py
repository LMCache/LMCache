...
class TestStoreGpuThroughput:
    """``l0_l1_store_gpu_throughput`` divides by (window - reserve)."""

    GPU_METRIC = "lmcache_mp.l0_l1_store_gpu_throughput"
    RESERVE_METRIC = "lmcache_mp.l0_l1_store_reserve_time"

    def test_uses_reserve_corrected_window(self, subscriber):
        count_before = _total_count(self.GPU_METRIC)
        sum_before = _sum(self.GPU_METRIC)
        subs = subscriber.get_subscriptions()
        subs[EventType.MP_STORE_START](
            _start_event(EventType.MP_STORE_START, "gpu-req", 10.0)
        )
        end = _end_event(EventType.MP_STORE_END, "gpu-req", 12.0, total_bytes=10**9)
        end.metadata["reserve_seconds"] = 1.0
        subs[EventType.MP_STORE_END](end)
        assert _total_count(self.GPU_METRIC) == count_before + 1
        # 1 GB over a (2.0 - 1.0) s corrected window -> 1.0 GB/s.
        assert _sum(self.GPU_METRIC) == pytest.approx(sum_before + 1.0)

    def test_skipped_when_reserve_swallows_window(self, subscriber):
        gpu_before = _total_count(self.GPU_METRIC)
        reserve_before = _total_count(self.RESERVE_METRIC)
        subs = subscriber.get_subscriptions()
        subs[EventType.MP_STORE_START](
            _start_event(EventType.MP_STORE_START, "swallow-req", 10.0)
        )
        end = _end_event(EventType.MP_STORE_END, "swallow-req", 11.0, total_bytes=10**9)
        end.metadata["reserve_seconds"] = 2.0
        subs[EventType.MP_STORE_END](end)
        assert _total_count(self.RESERVE_METRIC) == reserve_before + 1
        assert _total_count(self.GPU_METRIC) == gpu_before

    def test_skipped_without_matching_start(self, subscriber):
        gpu_before = _total_count(self.GPU_METRIC)
        reserve_before = _total_count(self.RESERVE_METRIC)
        end = _end_event(EventType.MP_STORE_END, "orphan-req", 12.0, total_bytes=10**9)
        end.metadata["reserve_seconds"] = 0.5
        subscriber.get_subscriptions()[EventType.MP_STORE_END](end)
        assert _total_count(self.RESERVE_METRIC) == reserve_before + 1
        assert _total_count(self.GPU_METRIC) == gpu_before
