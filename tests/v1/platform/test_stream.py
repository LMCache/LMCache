...
class TestMakeExternalStream:
    ...
    def test_delegates_to_cuda_backend_when_available(self):
        ...
        # ``lmcache.torch_device_type`` is frozen at import time, so on a
        # CPU-only host the dispatcher would otherwise pick the CPU
        # factory and never consult the CUDA backend.  Force the lookup
        # to ``cuda`` so this hit-path is exercised regardless of host.
        with (
            mock.patch.object(torch.cuda, "is_available", return_value=True),
            mock.patch(
