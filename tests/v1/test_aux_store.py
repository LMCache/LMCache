...
class FakeStorageManager:
    ...
    def __init__(self) -> None:
        ...
        self.prefetch_specs: list = []  # PrefetchRequestSpec per submit
    ...
    # --- fetch path ---
    def submit_prefetch_task(self, spec, **kw):
        # One PrefetchRequestSpec bundles keys + group_layout_descs + policy.
        self.prefetch_specs.append(spec)
...
def test_fetch_copies_chunks_and_releases_locks(store, sm, monkeypatch):
    ...
    # The prefetch went out as one spec carrying the single-group layout keyed
    # by object-group index (PrefetchRequestSpec validates this), SPARSE policy.
    (spec,) = sm.prefetch_specs
    assert spec.keys == keys
    assert list(spec.group_layout_descs) == [0]
    assert spec.group_layout_descs[0].shapes == [torch.Size([4])]  # 8 bytes bf16
