...
#         world_size=8,
#         worker_id=0,
#         chunk_hash=int("e3229141e680fb413d2c5d3ebb416c4ad300d381e309fc9e417757b91406c157", 16),
#         dtype=torch.uint8,
#     )
...
# @pytest.mark.skip(reason="GDS backend does not support memory fragmentation testing - free() requires MemoryObj not CacheEngineKey")
# def test_gds_backend_fragmentation():
#     """
#     Test GDS backend with fragmented allocations.
#     Allocate and free memory in a way that leaves gaps (fragmentation),
#     then allocate new memory and check if it reuses the freed gaps.
#     """
#     BASE_DIR = Path(__file__).parent
#     GDS_DIR = "/tmp/gds/test-cache-frag"
#     BACKEND_NAME = "GdsBackend"
#     BLOCK_SHAPE = [2048, 2048]  # ~4MB per block (uint8)
...
#         os.makedirs(GDS_DIR, exist_ok=True)
#         config_gds = LMCacheEngineConfig.from_file(BASE_DIR / "data/gds.yaml")
#         # NOTE: max_gds_size is in GB, so 32MB = 32 / 1024 GB
#         config_gds.max_gds_size = 32 / 1024  # 32MB in GB (Allow up to 8 blocks)
#         # Disable cufile to avoid hardware-specific issues
#         config_gds.extra_config = {"use_cufile": False}
...
#         # Allocate 4 blocks
#         for i in range(4):
#             key = CacheEngineKey(fmt="vllm", model_name=f"frag-{i}", world_size=1, worker_id=0, chunk_hash=i, dtype=torch.uint8)
#             keys.append(key)
#             memory_obj = gds_backend.memory_allocator.allocate(
#                 BLOCK_SHAPE, dtype=torch.uint8
#             )
...
#         # Free block 1 and block 3 (leave gaps)
#         gds_backend.memory_allocator.free(keys[1])
#         gds_backend.memory_allocator.free(keys[3])

#         # Allocate 2 more blocks, should reuse freed gaps
#         for i in range(2, 4):
#             key = CacheEngineKey(fmt="vllm", model_name=f"frag-new-{i}", world_size=1, worker_id=0, chunk_hash=i, dtype=torch.uint8)
#             memory_obj = gds_backend.memory_allocator.allocate(
#                 BLOCK_SHAPE, dtype=torch.uint8
#             )
#             future = gds_backend.submit_put_task(key, memory_obj)
...
#         # Verify: all keys should still be in cache
#         for i, key in enumerate(keys):
#             assert gds_backend.contains(key, False), f"Key {i} should remain in cache!"
