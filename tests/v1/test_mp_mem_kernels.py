...
FMT_HND = lmc_ops.GPUKVFormat.NL_X_TWO_NB_NH_BS_HS
FMT_HND_FLASH_INFER = lmc_ops.GPUKVFormat.NL_X_NB_TWO_NH_BS_HS
...
FORMAT_PARAMS = [
    ...
    (FMT_HND, 4, 8, 128, False),
    (FMT_HND_FLASH_INFER, 4, 8, 128, False),
...
def create_vllm_tensors(
    ...
    if gpu_kv_format == FMT_NORMAL:
        ...
        shape = [nb, 2, bs, nh, hs]
        return [_create_random_tensor(shape, dtype, device) for _ in range(nl)]
    elif gpu_kv_format == FMT_HND:
        shape = [2, nb, nh, bs, hs]
        return [_create_random_tensor(shape, dtype, device) for _ in range(nl)]
    elif gpu_kv_format == FMT_HND_FLASH_INFER:
        shape = [nb, 2, nh, bs, hs]
        return [_create_random_tensor(shape, dtype, device) for _ in range(nl)]
