...
def test_fresh_batch_is_not_orphaned() -> None:
    registry = LazyOffloadRequestRegistry()
    registry.arrive("req")
    registry.register_batch("req", 10, 0, [1, 2])

    assert registry.has_in_flight("req")
    assert registry.in_flight_request_ids() == {"req"}
    assert registry.in_flight_operation_ids() == {10}
    assert not registry.in_flight_is_orphaned(10)


def test_reset_orphans_the_in_flight_batch() -> None:
    """A preemption reset detaches the batch but never drops its pins."""
    registry = LazyOffloadRequestRegistry()
    registry.register_batch("req", 10, 0, [1])
    assert not registry.in_flight_is_orphaned(10)

    registry.reset("req")

    assert registry.has_in_flight("req")
    assert registry.in_flight_is_orphaned(10)
...
def test_rearrival_after_reset_keeps_a_new_batch_fresh() -> None:
    ...
    stale_request_id, stale = registry.complete_batch(10)
    assert stale_request_id == "req"
    assert stale.block_ids == (1,)
    assert stale.orphaned

    registry.register_batch("req", 11, 0, [2])
    assert not registry.in_flight_is_orphaned(11)
...
def test_finished_id_reuse_orphans_the_outstanding_batch() -> None:
    ...
    assert not registry.is_finished("req")
    assert registry.in_flight_is_orphaned(10)
    request_id, batch = registry.complete_batch(10)
...
def test_multiple_group_batches_can_overlap() -> None:
    registry = LazyOffloadRequestRegistry()
    registry.register_batch("req", 10, 0, [1])
    registry.register_batch("req", 11, 1, [2])

    assert registry.in_flight_operation_ids() == {10, 11}
    assert registry.in_flight_retention_groups() == {("req", 0), ("req", 1)}


def test_duplicate_operation_ids_are_rejected() -> None:
    registry = LazyOffloadRequestRegistry()
    registry.register_batch("req-a", 10, 0, [1])

    with pytest.raises(RuntimeError, match="operation 10 is already in flight"):
        registry.register_batch("req-b", 10, 1, [2])


def test_complete_batch_requires_an_open_batch() -> None:
    registry = LazyOffloadRequestRegistry()
    with pytest.raises(KeyError):
        registry.complete_batch(999)
    registry.arrive("req")
    with pytest.raises(KeyError):
        registry.complete_batch(999)
