...
def _make_manager_harness(
    ...
    return _Harness(pool=pool, manager=manager, ended_sessions=[])
...
class TestRealBlockPool:
    ...
    def test_manager_drains_and_completes_against_a_real_vllm_pool(self) -> None:
        ...
        actions = manager.on_store_results(set(), {"req": 1})
...
class _RoguePolicy(OffloadPolicy):
    ...
    def add(self, meta: LMCacheMPRequestMetadata, block_hashes: BlockHashes) -> None:
...
class TestDrainWiring:
    ...
    def test_drain_coalesces_one_request_into_one_store_op(self) -> None:
        """The worker tracks one in-flight store per request, so a drained batch must
        arrive as a single coalesced operation.
        """
        harness = _make_manager_harness()
    ...
    def test_emitting_a_blocked_request_is_a_logic_error(
        ...
        item.metadatas.append(
            (_make_store_metadata("req", [[1]], 0, 16), {1: b"hash-1"})
        )
        monkeypatch.setattr(
            manager_module,
