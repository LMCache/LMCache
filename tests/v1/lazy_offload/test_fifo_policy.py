...
class FakeStoreMetadata:
    """Minimal group-aware store metadata used by the policy tests."""
...
def make_meta(
    request_id: str = "req-0", retention_group_id: int = 0
) -> "LMCacheMPRequestMetadata":
    """Build a mock ``LMCacheMPRequestMetadata`` for one store operation."""
    return cast(
        "LMCacheMPRequestMetadata",
        FakeStoreMetadata(request_id, retention_group_id),
    )


def make_block_hashes(block_ids: list[int], engine_group_id: int = 0) -> BlockHashes:
    """Build a hash snapshot for the given block ids."""
    return {
        (engine_group_id, block_id): f"hash-{block_id}".encode()
...
class TestCoalescingAndSnapshots:
    ...
    def test_add_keeps_the_hash_snapshot_untouched(self) -> None:
        ...
        assert item.metadatas == [(meta, {(0, 0): b"hash-0", (0, 1): b"hash-1"})]

    def test_groups_of_one_request_drain_independently(self) -> None:
        policy = FIFOOffloadPolicy({THRESHOLD_KEY: 1})
        policy.add(make_meta("req", 0), make_block_hashes([0]))
        policy.add(make_meta("req", 1), make_block_hashes([1], 1))
        ...
        assert [item.retention_group_id for item in first.items] == [0, 1]
        assert first.emptied_request_ids == ["req"]

    def test_blocked_group_leaves_other_group_eligible(self) -> None:
        policy = FIFOOffloadPolicy({THRESHOLD_KEY: 1})
        policy.add(make_meta("req", 0), make_block_hashes([0]))
        policy.add(make_meta("req", 1), make_block_hashes([1], 1))
        signals = DrainSignals(
            ...
            blocked_retention_groups={("req", 0)},
        )

        first = policy.drain(signals)

        assert [item.retention_group_id for item in first.items] == [1]
        assert first.emptied_request_ids == []
        assert policy.has_pending_request("req")

    def test_emptied_request_order_follows_admission_order(self) -> None:
        policy = FIFOOffloadPolicy({THRESHOLD_KEY: 2})
        policy.add(make_meta("req-b"), make_block_hashes([0]))
        policy.add(make_meta("req-a"), make_block_hashes([1]))

        result = drain(policy, {"req-a", "req-b"})

        assert result.emptied_request_ids == ["req-b", "req-a"]
