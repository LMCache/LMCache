...
def add_op(
    ...
    *,
    ...
    meta = cast(
        ...
        FakeStoreMetadata(
            request_id,
            FakeStoreOp(start_tokens, end_tokens),
            retention_group_id,
        ),
    )
    hashes = cast(
        BlockHashes,
        {
            (engine_group_id, block_id): pool.blocks[block_id].block_hash
...
def add_unhashed_op(
    ...
    queue.add(meta, cast(BlockHashes, {(0, 999): None}))
...
def drain(
    ...
    blocked_groups: frozenset[tuple[str, int]] = frozenset(),
    ...
    return queue.drain(
        DrainSignals(
            ...
            blocked_retention_groups=set(blocked_groups),
            request_progress_tokens=progress or {},
        )
    )
...
class TestAdmission:
    ...
    def test_rejects_op_with_unhashed_block(self) -> None:
        ...
        queue.add(
            ...
            cast(
                BlockHashes,
                {(0, 1): pool.blocks[1].block_hash, (0, 2): None},
            ),
        )
...
class TestHybridRetentionGroups:
    def test_inflight_group_does_not_block_an_independent_group(self) -> None:
        pool = FakeBlockPool()
        pool.seed_free([1, 2])
        queue = make_queue(pool, horizon_steps=1.0)
        add_op(queue, pool, "req", [1], 256, retention_group_id=0)
        add_op(
            queue,
            pool,
            "req",
        ...
        result = drain(
            queue,
            new_blocks=2,
            blocked_groups=frozenset({("req", 0)}),
        )

        assert [
            (item.request_id, item.retention_group_id) for item in result.items
        ] == [("req", 1)]
        assert queue.has_pending_request("req")

    def test_full_attention_failure_breaks_only_its_group(self) -> None:
        pool = FakeBlockPool()
        pool.seed_held([1, 2, 3, 4])
        queue = make_queue(pool)
        add_op(queue, pool, "req", [1], 256, retention_group_id=0)
        add_op(
            queue,
        ...
        assert queue.mark_store_failed("req", 0, requires_prefix=True) == 1
        add_op(queue, pool, "req", [3], 512, retention_group_id=0)
        add_op(
            queue,
            pool,
            "req",
        ...
        result = drain(queue, progress={"req": 512})
        assert [item.retention_group_id for item in result.items] == [1]
        assert emitted_ends(result) == [256, 512]
        assert counters(queue).rejected_prefix_broken == 1

    def test_windowed_group_recovers_after_one_stale_range(self) -> None:
        pool = FakeBlockPool()
        pool.seed_free([1, 2])
        queue = make_queue(pool)
        add_op(
        ...
        assert emitted_ends(result) == [512]
        assert result.items[0].retention_group_id == 1
        assert counters(queue).dropped_evicted == 1
        assert counters(queue).rejected_prefix_broken == 0

    def test_recurrent_snapshot_drains_at_its_boundary(self) -> None:
        pool = FakeBlockPool()
        pool.seed_held([1])
        queue = make_queue(pool)
        add_op(
            ...
            [1],
            256,
            retention_group_id=2,
            engine_group_id=2,
            requires_prefix=False,
            retire_at_token=256,
        )

        assert drain(queue, progress={"req": 255}).items == []
        result = drain(queue, progress={"req": 256})
        assert [item.retention_group_id for item in result.items] == [2]
        assert emitted_ends(result) == [256]

    def test_duplicate_numeric_block_ids_remain_group_qualified(self) -> None:
        pool = FakeBlockPool()
        pool.seed_free([7])
        queue = make_queue(pool)
        meta = cast(
            "LMCacheMPRequestMetadata",
            FakeStoreMetadata("req", FakeStoreOp(0, 256), 0),
        )
        snapshot = pool.blocks[7].block_hash
        hashes = cast(BlockHashes, {(0, 7): snapshot, (1, 7): snapshot})
        queue.add(meta, hashes)

        result = drain(queue, new_blocks=1)

        assert len(result.items[0].metadatas[0][1]) == 2
        assert set(result.items[0].metadatas[0][1]) == {(0, 7), (1, 7)}
