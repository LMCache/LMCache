...
exists in cuda_ops has a matching signature.

Does NOT require torch_ops to implement everything in cuda_ops —
only checks the intersection. If you implement a function in the fallback,
its signature must match cuda_ops exactly.

Default value rules (relaxed):
  - If cuda_ops has a default, fallback MUST also have one with the same value —
    otherwise callers relying on the default will break.
  - If cuda_ops has NO default, fallback MAY add one (superset, backward-compatible).

Requires CUDA (cuda_ops must be importable). Automatically skipped on CPU-only CI.
"""
...
_cuda_ops_callables = _public_callables(cuda_ops) if HAS_CUDA_OPS else {}
_shared_func_names = sorted(
    (set(_fallback_callables) & set(_cuda_ops_callables)) - _EXCLUDED_FUNCS
)

_fallback_enums = _public_enums(fallback)
_cuda_ops_enums = _public_enums(cuda_ops) if HAS_CUDA_OPS else {}
_shared_enum_names = sorted(set(_fallback_enums) & set(_cuda_ops_enums))

_fallback_descs = _public_descriptor_classes(fallback)
_cuda_ops_descs = _public_descriptor_classes(cuda_ops) if HAS_CUDA_OPS else {}
_shared_desc_names = sorted(set(_fallback_descs) & set(_cuda_ops_descs))


# ── Tests ──


@pytest.mark.skipif(not HAS_CUDA_OPS, reason="cuda_ops not available (no CUDA)")
...
def test_function_signature_parity(func_name):
    ...
    When cuda_ops has real py::arg() names  → check names, count, defaults.
    When cuda_ops only has arg0/arg1/...    → check count and defaults only.

    Default rules (relaxed):
      cuda_ops has default, fallback does NOT      → FAIL (callers will break)
      cuda_ops has default, fallback has different → FAIL (silent behavior change)
      cuda_ops has NO default, fallback does       → OK   (backward-compatible)
    """
    if func_name == "__placeholder__":
        pytest.skip("No shared functions found between cuda_ops and fallback")

    c_func = _cuda_ops_callables[func_name]
    py_func = _fallback_callables[func_name]

    # Get cuda_ops params: try docstring first (more reliable for pybind11),
    ...
    if c_params is None:
        pytest.skip(f"Cannot inspect cuda_ops.{func_name} signature")
    ...
    for i, (
        ...
        if c_has_names:
            assert c_name == py_name, (
                ...
                f"cuda_ops: '{c_name}', fallback: '{py_name}'"
            )

        # Relaxed default check:
        # Only fail if cuda_ops has a default but fallback does NOT,
        ...
        if c_has_def:
            assert py_has_def, (
                ...
                f"cuda_ops has default={c_default!r} but fallback has NO default. "
                f"Callers relying on the default will break."
            )
            assert c_default == py_default, (
                f"{func_name}: param #{i} ('{py_name}') — "
                f"default value mismatch: "
                f"cuda_ops={c_default!r}, fallback={py_default!r}"
            )
...
def test_enum_parity(enum_name):
    ...
    its members and values must match cuda_ops."""
    if enum_name == "__placeholder__":
        pytest.skip("No shared enums found between cuda_ops and fallback")

    c_members = _get_enum_members(_cuda_ops_enums[enum_name])
...
# ── Coverage tests: every cuda_ops API must have a fallback ──


@pytest.mark.skipif(not HAS_CUDA_OPS, reason="cuda_ops not available (no CUDA)")
def test_all_cuda_ops_callables_have_fallback() -> None:
    """Every public callable in cuda_ops must exist in
    torch_ops."""
    missing = sorted(
        set(_cuda_ops_callables) - set(_fallback_callables) - _EXCLUDED_FUNCS
    )
    assert not missing, f"cuda_ops callables missing from torch_ops: {missing}"


@pytest.mark.skipif(not HAS_CUDA_OPS, reason="cuda_ops not available (no CUDA)")
def test_all_cuda_ops_enums_have_fallback() -> None:
    """Every public enum in cuda_ops must exist in
    torch_ops."""
    missing = sorted(set(_cuda_ops_enums) - set(_fallback_enums))
    assert not missing, f"cuda_ops enums missing from torch_ops: {missing}"


@pytest.mark.skipif(not HAS_CUDA_OPS, reason="cuda_ops not available (no CUDA)")
def test_all_cuda_ops_descriptors_have_fallback() -> None:
    """Every public descriptor class in cuda_ops must exist in
    torch_ops."""
    missing = sorted(set(_cuda_ops_descs) - set(_fallback_descs) - _EXCLUDED_DESCS)
    assert not missing, f"cuda_ops descriptor classes missing from torch_ops: {missing}"
...
def test_descriptor_class_parity(cls_name: str) -> None:
    """For every descriptor class shared between cuda_ops and the fallback,
    every public, non-callable attribute on the cuda_ops instance must also
    ...
    try:
        c_inst = _cuda_ops_descs[cls_name]()
    except TypeError:
        pytest.skip(f"{cls_name} has no no-arg constructor in cuda_ops")
    ...
    assert not missing, (
        f"{cls_name}: fallback is missing attributes present on cuda_ops: {missing}"
    )
