...
def test_lm_connector(url, autorelease_v1, lmserver_v1_process):
    ...
    # Create memory allocator and LocalCPUBackend properly
    ...
    try:
        ...
        get_memory_obj = future.result()
        assert get_memory_obj is not None
        check_mem_obj_equal(memory_obj, get_memory_obj)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            connector = autorelease_v1(
                CreateConnector(url, async_loop, local_cpu_backend)
            )
            random_key = dumb_cache_engine_key()
