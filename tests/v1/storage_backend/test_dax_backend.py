...
def test_dax_backend_batched_get_blocking_serializes_restore_dispatch(
    memory_allocator,
    loop_in_thread,
    monkeypatch,
) -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        key1 = CacheEngineKey("test_model", 1, 0, 41, torch.bfloat16)
        key2 = CacheEngineKey("test_model", 1, 0, 42, torch.bfloat16)
        ...
        try:
            _store_tensor(backend, key1, num_tokens=16, fill_value=10)
            _store_tensor(backend, key2, num_tokens=16, fill_value=11)

            with ThreadPoolExecutor(max_workers=2) as executor:
                first_future = executor.submit(backend.batched_get_blocking, [key1])
                assert first_restore_started.wait(timeout=1)

                second_future = executor.submit(backend.batched_get_blocking, [key2])
                assert not second_restore_started.wait(timeout=0.2)

                allow_first_restore.set()

                first_results = first_future.result(timeout=2)
                assert second_restore_started.wait(timeout=1)
                second_results = second_future.result(timeout=2)

            assert len(first_results) == 1
            assert len(second_results) == 1
            assert first_results[0] is not None
            assert second_results[0] is not None
            assert torch.all(first_results[0].tensor == 10)
            assert torch.all(second_results[0].tensor == 11)
            first_results[0].ref_count_down()
            second_results[0].ref_count_down()
        finally:
            backend.close()
