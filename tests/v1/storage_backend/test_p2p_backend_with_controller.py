...
class TestP2PBackendWithController:
    ...
    def test_p2p_backend_partial_hits(self, async_loop):
        ...
        This test verifies:
        - Partial hit scenarios are handled correctly
        - Only available data is retrieved
        - The system properly reports partial results
        - Mixed availability scenarios work as expected
        """
        peer1_init_port, peer1_lookup_port, peer2_init_port, peer2_lookup_port = (
            get_available_ports(4)
        )
        ...
        test_keys = [create_test_key(f"key_{i}") for i in range(3)]
        test_hashes = tuple([key.chunk_hash for key in test_keys])
        peer2_init_url = f"localhost:{peer2_init_port}"

        peer_mappings = {
            ("test_instance", 0, test_hashes): BatchedP2PLookupRetMsg(
                layout_info=[("test_instance", "cpu", 1, peer2_init_url)]
            )
        }
            ...
            with mock_controller_context(peer_mappings) as controller_url:
                with (
                    ...
                    with (
                        ...
                        if mem_obj.tensor is not None:
                            mem_obj.tensor.fill_(42.0)

                        local_backend2.submit_put_task(test_keys[0], mem_obj)

                        # Wait for the first key to be stored in cache
                        assert wait_for_keys_in_cache(local_backend2, [test_keys[0]]), (
                            "First key not stored in cache"
                        )

                        async def test_partial_hits():
                            num_hits = await p2p_backend1.batched_async_contains(
                                lookup_id="test_lookup_partial",
                                keys=test_keys,
                                pin=False,
                            )
                            assert num_hits == 1, f"Expected 1 hit, got {num_hits}"

                            cum_chunk_lengths = [0, 256, 512, 768]
                            retrieved_objs = (
                                await p2p_backend1.batched_get_non_blocking(
                                    lookup_id="test_lookup_partial",
                                    keys=test_keys,
                            ...
                            assert len(retrieved_objs) == 1, (
                                f"Expected 1 object, got {len(retrieved_objs)}"
                            )

                        asyncio.run_coroutine_threadsafe(
                            test_partial_hits(), async_loop
                        ).result(timeout=10)
