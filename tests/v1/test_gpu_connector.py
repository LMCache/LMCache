...
def test_vllm_paged_connector_v2_paged_attn_with_gpu(use_gpu):
    ...
    gpu_kv_src = generate_kv_cache_paged_list_tensors_for_paged_attn(
        num_blocks, device, block_size
    )
    gpu_kv_dst = generate_kv_cache_paged_list_tensors_for_paged_attn(
        num_blocks, device, block_size
    )
...
def test_vllm_paged_connector_v2_to_gpu_bench(benchmark):
    ...
    connector = VLLMPagedMemGPUConnectorV2(num_layers, num_heads, head_size, block_size)
