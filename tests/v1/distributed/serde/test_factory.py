...
def test_register_serde_factory_dispatch() -> None:
    ...
    class _DummyProcessor(SerdeProcessor):
        ...
        def submit_serialize(self, src_objs, dst_objs, keys=None):  # type: ignore[no-untyped-def]
            return 0

        def query_serialize_result(self, task_id):  # type: ignore[no-untyped-def]
            return True

        def submit_deserialize(self, src_objs, dst_objs, keys=None):  # type: ignore[no-untyped-def]
            return 0
