...
"""
Unit tests for L1StatsLogger.

Tests cover:
- L1 counter accumulation for each callback
- L1 latency tracking via FIFO deque (reserved → finished)
...
class TestL1CounterCallbacks:
    def test_reserved_read_increments_key_counter(self, logger):
        logger.on_l1_keys_reserved_read(make_keys(5))
        assert logger.stats.interval_l1_read_keys == 5

    def test_reserved_read_multiple_batches_accumulate(self, logger):
        logger.on_l1_keys_reserved_read(make_keys(3))
        logger.on_l1_keys_reserved_read(make_keys(4, offset=10))
        assert logger.stats.interval_l1_read_keys == 7

    def test_reserved_write_increments_key_counter(self, logger):
...
class TestL1LatencyTracking:
    def test_read_latency_is_positive(self, logger):
        logger.on_l1_keys_reserved_read(make_keys(2))
        time.sleep(0.001)
        logger.on_l1_keys_read_finished(make_keys(2))

        assert len(logger.stats.l1_read_latency) == 1
        assert logger.stats.l1_read_latency[0] > 0

    def test_write_latency_is_positive(self, logger):
        logger.on_l1_keys_reserved_write(make_keys(2))
        time.sleep(0.001)
        logger.on_l1_keys_write_finished(make_keys(2))

        assert len(logger.stats.l1_write_latency) == 1
        assert logger.stats.l1_write_latency[0] > 0

    def test_multiple_batches_record_multiple_latencies(self, logger):
        for _ in range(3):
            logger.on_l1_keys_reserved_read(make_keys(1))
            logger.on_l1_keys_read_finished(make_keys(1))

        assert len(logger.stats.l1_read_latency) == 3

    def test_fifo_ordering_for_read(self, logger):
        """Earlier batches must have smaller or equal latency than later ones
        (we sleep between batches to ensure ordering)."""
        logger.on_l1_keys_reserved_read(make_keys(1))
        time.sleep(0.005)
        logger.on_l1_keys_reserved_read(make_keys(1, offset=10))

        logger.on_l1_keys_read_finished(make_keys(1))
        time.sleep(0.005)
        logger.on_l1_keys_read_finished(make_keys(1, offset=10))

        latencies = logger.stats.l1_read_latency
        assert len(latencies) == 2
        assert all(lat > 0 for lat in latencies)

    def test_read_finished_without_prior_reserved_does_not_crash(self, logger):
        """Calling on_l1_keys_read_finished with an empty deque should be safe."""
        logger.on_l1_keys_read_finished(make_keys(2))
        assert len(logger.stats.l1_read_latency) == 0

    def test_write_finished_without_prior_reserved_does_not_crash(self, logger):
        logger.on_l1_keys_write_finished(make_keys(2))
        assert len(logger.stats.l1_write_latency) == 0
...
class TestLogPrometheus:
    def test_stats_are_reset_after_log_prometheus(self, logger):
        logger.on_l1_keys_reserved_read(make_keys(4))
    ...
    def test_l1_counters_forwarded_to_prometheus(self, logger):
        logger.on_l1_keys_reserved_read(make_keys(5))
        logger.on_l1_keys_reserved_write(make_keys(3))
        logger.on_l1_keys_deleted_by_manager(make_keys(2))
    ...
    def test_l1_latency_histograms_forwarded_to_prometheus(self, logger):
        ...
        assert logger._l1_read_latency_histogram.observe.call_count == 2

    def test_latency_deques_cleared_after_log_prometheus(self, logger):
        logger.on_l1_keys_reserved_read(make_keys(1))
        logger.on_l1_keys_read_finished(make_keys(1))
        ...
        logger.log_prometheus()
        assert logger._l1_read_latency_histogram.observe.call_count == 1
