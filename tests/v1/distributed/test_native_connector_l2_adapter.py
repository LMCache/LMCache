...
class TestDeleteRespectsLocks:
    """Verify lookup locks protect keys from concurrent deletion."""

    def test_delete_skips_locked_key(self, adapter: NativeConnectorL2Adapter) -> None:
        key = create_object_key(1)
    ...
    def test_delete_locked_key_succeeds_after_unlock(
    ...
    def test_delete_partial_lock_batch(
        self, adapter_with_capacity: NativeConnectorL2Adapter
    ) -> None:
        adp = adapter_with_capacity
        keys = [create_object_key(i) for i in range(3)]
        # Distinct sizes expose positional accounting errors.
        sizes = [50, 100, 150]
        objs: list[MemoryObj] = [
    ...
    def test_delete_locked_key_preserves_usage(
        self, adapter_with_capacity: NativeConnectorL2Adapter
    ) -> None:
        adp = adapter_with_capacity
        key = create_object_key(1)
