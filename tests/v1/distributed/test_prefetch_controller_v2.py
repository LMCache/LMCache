...
"""
Unit tests for the v2 PrefetchController.
...
def wait_for_result(
    ...
    """Poll query_prefetch_result until it returns the result grid."""
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
...
def row_bits(result: Bitmap2D | None, row: int = 0) -> list[int]:
    """Return the set chunk indices of one row of a result, failing on None."""
    assert result is not None, "prefetch did not complete"
    return result[row].get_indices_list()


def hit_length(result: Bitmap2D | None) -> int:
    """Return the leading-ones prefix length of row 0 of a result."""
    assert result is not None, "prefetch did not complete"
