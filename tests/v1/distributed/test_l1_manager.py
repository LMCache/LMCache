...
# =============================================================================
# Tests for L1Manager.pin() / unpin()
# =============================================================================


class TestPin:
    """Tests for L1Manager.pin() and unpin()."""

    def _make_ready(self, manager, layout, chunk_hash: int):
        """Create a ready (unlocked) object and return its key."""
        key = make_object_key(chunk_hash)
        manager.reserve_write([key], [False], layout)
        manager.finish_write([key])
        return key

    def test_pin_missing_key_returns_key_not_exist(self, basic_l1_config, basic_layout):
        """Pinning a non-resident key reports KEY_NOT_EXIST (best-effort)."""
        manager = L1Manager(basic_l1_config)
        key = make_object_key(999)

        assert manager.pin([key]) == {key: L1Error.KEY_NOT_EXIST}

        manager.close()

    def test_pin_makes_ready_key_not_evictable(self, basic_l1_config, basic_layout):
        """A pinned ready key is protected from eviction; unpin restores it."""
        manager = L1Manager(basic_l1_config)
        key = self._make_ready(manager, basic_layout, 1)
        assert manager.is_key_evictable(key) is True

        assert manager.pin([key]) == {key: L1Error.SUCCESS}
        assert manager.is_key_evictable(key) is False

        assert manager.unpin([key]) == {key: L1Error.SUCCESS}
        assert manager.is_key_evictable(key) is True

        manager.close()

    def test_pin_is_reference_counted(self, basic_l1_config, basic_layout):
        """Two pins require two unpins before the key is evictable again."""
        manager = L1Manager(basic_l1_config)
        key = self._make_ready(manager, basic_layout, 1)
        ...
        manager.unpin([key])
        assert manager.is_key_evictable(key) is False  # still pinned once
    ...
    def test_unpin_non_pinned_key_reports_wrong_state(
        self, basic_l1_config, basic_layout
    ):
        """Unpinning a non-pinned key floors at 0 and reports KEY_IN_WRONG_STATE
        (so it is not counted as a released pin)."""
        manager = L1Manager(basic_l1_config)
        key = self._make_ready(manager, basic_layout, 1)

        assert manager.unpin([key]) == {key: L1Error.KEY_IN_WRONG_STATE}
        assert manager.is_key_evictable(key) is True

        # A subsequent pin still takes exactly one unpin to release.
        manager.pin([key])
        assert manager.is_key_evictable(key) is False
        assert manager.unpin([key]) == {key: L1Error.SUCCESS}
        assert manager.is_key_evictable(key) is True
    ...
    def test_delete_rejects_pinned_key(self, basic_l1_config, basic_layout):
        """delete() must not remove a pinned key (closes the eviction race
        where a key is pinned between is_key_evictable() and delete())."""
        manager = L1Manager(basic_l1_config)
        key = self._make_ready(manager, basic_layout, 1)
        manager.pin([key])
    ...
    def test_clear_preserves_pinned_but_force_drops_them(
        self, basic_l1_config, basic_layout
    ):
        """clear(force=False) keeps pinned objects; clear(force=True) drops them."""
        manager = L1Manager(basic_l1_config)
        key = self._make_ready(manager, basic_layout, 1)
        manager.pin([key])

        manager.clear(force=False)
        assert manager.get_object_state(key) is not None
    ...
    def test_report_status_counts_pinned(self, basic_l1_config, basic_layout):
        """report_status() surfaces the number of pinned objects."""
        manager = L1Manager(basic_l1_config)
        key = self._make_ready(manager, basic_layout, 1)

        assert manager.report_status()["pinned_count"] == 0
        manager.pin([key])
        assert manager.report_status()["pinned_count"] == 1
        manager.unpin([key])
        assert manager.report_status()["pinned_count"] == 0
