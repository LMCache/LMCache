...
class TestRecovery:
    ...
    def test_partial_delete_failure_untracks_successful_paths(
        self,
        tmp_path: Path,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        key = create_object_key(1)
        current_path = tmp_path / _object_key_to_filename(key)
        legacy_filename = _legacy_object_key_to_filename(key)
        assert legacy_filename is not None
        legacy_path = tmp_path / legacy_filename
        current_size = 17
        legacy_size = 23
        current_path.write_bytes(b"c" * current_size)
        legacy_path.write_bytes(b"l" * legacy_size)

        reopened = FSL2Adapter(
            FSL2AdapterConfig(base_path=str(tmp_path), max_capacity_gb=0.001)
        )
        try:
            assert reopened.get_usage().total_bytes_used == current_size + legacy_size

            original_unlink = fs_module.aiofiles.os.unlink

            async def fail_current_path(path: Path) -> None:
                if Path(path) == current_path:
                    raise OSError("injected delete failure")
                await original_unlink(path)

            monkeypatch.setattr(fs_module.aiofiles.os, "unlink", fail_current_path)

            reopened.delete([key])

            assert current_path.exists()
            assert not legacy_path.exists()
            assert reopened.get_usage().total_bytes_used == current_size
        finally:
            reopened.close()
