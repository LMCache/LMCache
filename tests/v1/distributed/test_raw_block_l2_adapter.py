...
@requires_raw_block_ext
def test_raw_block_l2_adapter_v2_incremental_multi_key_recovery():
    """End-to-end MP path test for v2 base+delta checkpoint recovery.

    Mirrors the non-MP integration in
    ``test_rust_raw_block_backend_v2_incremental_multi_key_recovery`` but
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        config = RawBlockL2AdapterConfig(
            ...
            meta_delta_high_watermark_pct=95,
        )
        ...
        adapter1 = RawBlockL2Adapter(config)
        try:
            assert adapter1._core._use_incremental is True
            assert adapter1._core._meta_layout.delta_region_bytes > 0
...
def test_raw_block_l2_adapter_close_flushes_pending_via_delta():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        adapter1 = RawBlockL2Adapter(config)
        try:
            # First store + explicit checkpoint becomes the v2 full base.
