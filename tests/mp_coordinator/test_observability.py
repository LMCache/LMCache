...
from lmcache.mp_coordinator.observability import init_coordinator_metrics


def test_disabled_metrics_are_not_initialized() -> None:
    config = MPCoordinatorConfig(metrics_enabled=False)

    with patch("lmcache.mp_coordinator.observability.init_otel_metrics") as mock_init:
