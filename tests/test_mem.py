...
def test_fast_mem_load():
    ...
    kv_shape = [num_blk, blk_size, 8, 128]
    slot_mapping = random.sample(range(0, num_tok+1), num_load)
    kv_cache = [torch.rand(kv_shape, dtype=kv_dtype, device='cuda'),
    ...
    key_cache = kv_caches[0].reshape(-1, num_heads, head_size)
    value_cache = kv_caches[1].reshape(-1, num_heads, head_size)
