...
def test_gpu_basic_operations():
    ...
    try:
        ...
        print(f"GPU memory after tensor: {gpu_memory:.1f} MB")
        ...
        gpu_memory_cleanup = torch.cuda.memory_allocated(0) / 1024**2
        print(f"GPU memory after cleanup: {gpu_memory_cleanup:.1f} MB")
...
def test_gpu_hugepage_memory_transfer():
    ...
    try:
        ...
        # 데이터 일치성 확인
        assert torch.allclose(gpu_tensor, gpu_tensor2), (
            "Data mismatch after round-trip transfer"
        )
...
def test_gpu_hugepage_stress():
    ...
    try:
        ...
        for i in range(5):
            ...
            gpu_memory = torch.cuda.memory_allocated(0) / 1024**2
            print(f"  Tensor {i + 1}: {tensor.shape}, GPU memory: {gpu_memory:.1f} MB")
