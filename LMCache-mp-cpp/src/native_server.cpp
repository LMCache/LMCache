  ...
  while (!stop_.load()) {
    ...
    if ((items[0].revents & ZMQ_POLLIN) != 0) {
      ...
      if (frames.size() >= 3) {
        ...
        if (TryHandleImmediateRequest(task, &fast_response)) {
          (void)SendMultipart(socket, fast_response);
          sent_direct_response = true;
        } else if (TryHandleReadyLookupResult(task, &fast_response)) {
          (void)SendMultipart(socket, fast_response);
          sent_direct_response = true;
        } else {
...
bool NativeServer::TryHandleImmediateRequest(const RequestTask& task,
                                             Frames* response) {
  const auto start = std::chrono::steady_clock::now();
  const bool record_metrics = config_.enable_http;
  *response = task.prefix;
  switch (static_cast<RequestType>(task.request_type)) {
  ...
  // Direct replies avoid worker handoff for state-free control traffic.
  if (record_metrics) {
...
void NativeServer::EnqueueTask(RequestTask task) {
  task.enqueued_at = std::chrono::steady_clock::now();
  {
    ...
    if (tasks_.size() >= config_.max_queued_tasks) {
...
bool NativeServer::TryHandleReadyLookupResult(const RequestTask& task,
  ...
  out.push_back(msgpack::EncodeUnsigned(result));
  RecordRequestLatency(task.request_type, ElapsedMicros(start));
