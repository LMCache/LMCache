...
class NixlPipe:
    ...
    def __init__(self, nixl_config: NixlConfig, side_channel: zmq.Socket):
    ...
    def commit_write(self, write_size: int, uid: str) -> str:
        ...
        # Wait for the remote peer to acknowledge the transfer and return the new uuid
    ...
    def wait_read(self, uid: str):
        ...
        while True:
            if self._agent.check_remote_xfer_done(self.peer_name, message.encode("utf-8")):
                logger.info("Transfer for UUID '%s' completed on the remote side (%s)",
                    uid, self.peer_name)
                break
            time.sleep(0.001)
...
class NixlChannel:
    ...
    def __init__(self, nixl_config: NixlConfig):
        ...
        self._side_channel: zmq.Socket = self._context.socket(zmq.PAIR)
        ...
        if nixl_config.role == NixlRole.SENDER:
            ...
            self._side_channel.setsockopt(
                zmq.RCVTIMEO, 5000  # Set a timeout for receiving to avoid blocking indefinitely
            )
