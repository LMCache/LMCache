...
from lmcache.experimental.cache_controller.message import (
    RegisterMsg, DeRegisterMsg)
from lmcache.experimental.cache_controller.rpc_utils import (
    get_zmq_context, get_zmq_socket, close_zmq_socket)
...
class RegistrationController:
    ...
    def get_socket(
        self, instance_id: str, worker_id: int) -> Optional[zmq.asyncio.Socket]:
        """
        ...
        if socket is None:
            logger.warning(
                f"Instance-worker {(instance_id, worker_id)}"
                "not registered")
        return socket
    ...
    async def register(
        ...
        logger.info(
            f"Registered instance-worker {(instance_id, worker_id)}"
            f"with URL {url}")
    
    async def deregister(
        self, msg: DeRegisterMsg) -> None:
        """
