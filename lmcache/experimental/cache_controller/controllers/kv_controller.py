from lmcache.experimental.cache_controller.message import (
    KVAdmitMsg, KVEvictMsg, ClearMsg, LookupMsg, LookupRetMsg,
    ClearRetMsg)
...
class KVController:
    ...
    async def admit(self, msg: KVAdmitMsg) -> None:
        ...
        if instance_id not in self.kv_pool:
            self.kv_pool[key] = []
        self.kv_pool[key].append(KVChunkMetadata(
                                instance_id,
                                worker_id,
                                location
                            ))
    ...
    async def evict(self, msg: KVEvictMsg) -> None:
        ...
        remaining = [
            ...
            if not (m.instance_id == instance_id
    ...
    # suffix chunk is still in the system. LMCache should gurantee
