...
from rpc_utils import get_client_socket, get_zmq_context, get_unix_socket_path, clean_old_sockets
...
# NOTE(Jiayi): Also, consider scaling up the number of cluster executors in the future.
class LMCacheClusterExecutor:
    ...
    def __init__(self, lmcache_instance_ids: List[str]):
        ...
        """
        assert len(lmcache_instance_ids) == 1, "Multiple lmcache instances are not supported for now"
        
        lmcache_instance_id = lmcache_instance_ids[0]
        
        # TODO(Jiayi): need to make this an argument
        self.instance_metadata_path = f"/tmp/lmcache_instance_metadata_{lmcache_instance_id}.json"
    ...
    async def _monitor_metadata_loop(self, poll_interval: float = 10.0):
        while True:
            try:
        ...
        try:
            ...
            for instance_id, worker_ids in instance_metadata.items():
                if instance_id not in self.instances:
                    ...
                    logger.info(
                        f"Added instance {instance_id}, "
                        f"worker_ids: {worker_ids}")
                    continue
                if Counter(self.instances[instance_id].worker_ids) != Counter(worker_ids):
                    # TODO(Jiayi): We can make this incremental instead of re-initializing
                    async with self.instance_lock:
                        self.instances[instance_id] = LMCacheInstanceExecutor(
                            instance_id, worker_ids)
                    logger.info(
                        f"Updated instance {instance_id}, "
                        f"worker_ids: {worker_ids}")
        except Exception as e:
    ...
    async def execute(self, instance_id, operation, **kwargs):
        ...
        :param operation: The operation to execute (e.g., 'lookup', 'clear_all').
...
class LMCacheInstanceExecutor:
    ...
    def __init__(self, instance_id:str, worker_ids: List[int]):
        ...
        for worker_id in worker_ids:
            ...
            self.sockets[worker_id] = get_client_socket(self.context, socket_path)
    ...
    async def lookup(self, **kwargs):
        ...
        res = await self.execute_worker("lookup", worker_ids, tokens=kwargs["tokens"])
  
        return res
    
    async def execute_worker(self, operation: str, worker_ids: List[int], **kwargs):
        """
        ...
        tasks = []
        for worker_id in worker_ids:
            scoket = self.sockets[worker_id]
            task = await scoket.send_json({"operation": operation, "data": kwargs})
            tasks.append(task)
        results = await asyncio.gather(*tasks)
        return {worker_id: result for worker_id, result in zip(worker_ids, results)}
