...
from lmcache.experimental.cache_controller.message import (
    MsgBase, WorkerMsg, ControlMsg, OrchMsg, RegisterMsg,
    DeRegisterMsg, KVAdmitMsg, KVEvictMsg, LookupMsg,
    ClearMsg, OrchRetMsg, ControlRetMsg,
    ClearRetMsg)
...
class LMCacheControllerManager:
    ...
    async def handle_orchestration_message(
        ...
        if isinstance(msg, LookupMsg):
            ...
            logger.error(
                f"Unknown ochestration message type: {msg}")
            return None
                
    async def handle_batched_request(
        self, 
        socket
    ) -> Optional[MsgBase]:
        while True:
