...
"""Shared filesystem filename codec for L2 adapters."""
...
def object_key_to_filename(key: ObjectKey) -> str:
    ...
    Returns:
        A ``.data`` filename containing every field of ``key``.
    """
...
def filename_to_object_key(filename: str) -> Optional[ObjectKey]:
    ...
    Returns:
        The decoded key, or ``None`` when ``filename`` is not a valid
