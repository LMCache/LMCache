...
# Lightweight installs (e.g. lmcache-cli) ship a lmache_native without
...
try:
    # First Party
    from lmcache.lmache_native import fold as _native_fold
    from lmcache.lmache_native import unfold as _native_unfold
except ImportError:
    _native_fold = None
    _native_unfold = None
...
def fold(
    ...
    Raises:
        ...
        ImportError: If the installed ``lmcache.lmache_native`` does not
            provide the fold kernel (lightweight install).
    """
    if _native_fold is None:
        raise ImportError(
            "lmcache.lmache_native lacks the fold kernel; install or "
...
def unfold(
    ...
    if _native_unfold is None:
        raise ImportError(
            "lmcache.lmache_native lacks the unfold kernel; install or "
            "build the full lmcache package"
        )
    if num_ranks < 1:
        raise ValueError(f"num_ranks must be >= 1 (got {num_ranks})")
