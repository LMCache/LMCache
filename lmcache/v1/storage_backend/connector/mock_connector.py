...
class PressureManager:
    ...
    async def on_put(self, mock_obj: MockMemoryObj):
        ...
        logger.debug(
            "waiting %s seconds to put %d bytes", total_wait_time, mock_obj.num_bytes
        )
    ...
    async def on_get(self, mock_obj: MockMemoryObj):
        ...
        logger.debug(
            "waiting %s seconds to get %d bytes", total_wait_time, mock_obj.num_bytes
        )
    ...
    async def on_batched_get(self, mock_objs: List[Optional[MockMemoryObj]]):
        ...
        logger.debug("waiting %s seconds to get %d bytes", total_wait_time, total_bytes)
        async with self.read_lock:
            await asyncio.sleep(total_wait_time)
