...
def _get_allocator_buffer_info(
    ...
    if isinstance(allocator, (MixedMemoryAllocator, PinMemoryAllocator)):
        ...
        return buf.data_ptr(), buf.nbytes
...
class CuObjectS3Connector(S3Connector):
    ...
    def _rdma_upload(self, key_str: str, memory_obj: MemoryObj) -> s3.S3Request:
        ...
        # Actual data is transferred via server-initiated RDMA_READ
        ...
        def on_headers(status_code, headers, **kwargs):
            ...
            logger.debug(f"_rdma_upload status: {status_code}")
    ...
    async def close(self) -> None:
        """Drain in-flight CRT requests, then release RDMA resources.

        ``super().close()`` calls ``pq_executor.shutdown(wait=True)``
        which blocks until every queued job (including CRT requests
        carrying RDMA tokens) has finished.  Only after that drain is
        it safe to deregister the memory pool and destroy the cuObject
        client -- otherwise a server-initiated ``RDMA_READ``/``WRITE``
        could target memory whose registration has been revoked.
        """
        # 1. Wait for all in-flight RDMA transfers to complete.
        await super().close()

        # 2. Now safe to tear down cuObject resources.
