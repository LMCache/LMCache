...
class BenchmarkMemoryObj: 
    ...
    def from_tensor_memory_obj(tensor_memory_obj: MemoryObj) -> "BenchmarkMemoryObj": 
        ...
        return BenchmarkMemoryObj(
            tensor=tensor_memory_obj.tensor.detach().clone(), 
...
class AsyncLRU: 
    """
    the async lock protects against race conditions while mimicing synchronization
    ...
    async def put(self, key: CacheEngineKey, benchmark_obj: BenchmarkMemoryObj): 
        async with self.lock: 
            ...
            if alloc_size > self.capacity: 
                raise ValueError(f"Allocation size {alloc_size} is greater than capacity {self.capacity}")
            if key in self.dict: 
                self.dict.move_to_end(key)
                return None
...
class PressureManager: 
    ...
    async def on_put(self, benchmark_obj: BenchmarkMemoryObj): 
        ...
        logger.debug(f"waiting {total_wait_time} seconds to put {benchmark_obj.num_bytes} bytes")
    ...
    async def on_get(self, benchmark_obj: BenchmarkMemoryObj):
        ...
        logger.debug(f"waiting {total_wait_time} seconds to get {benchmark_obj.num_bytes} bytes")
...
class BenchmarkConnector(RemoteConnector): 
    ...
    def exists_sync(self, key: CacheEngineKey) -> bool:
        raise NotImplementedError("BenchmarkConnector does not support synchronous exists")
