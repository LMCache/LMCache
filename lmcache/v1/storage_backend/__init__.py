...
def CreateStorageBackends(
    ...
    if metadata.role == "scheduler":
        # For scheduler role, use DummyLocalCpuBackend
        local_cpu_backend = DummyLocalCpuBackend(
            config,
        )
        backend_name = str(local_cpu_backend)
        storage_backends["LocalCPUBackend"] = local_cpu_backend
    ...
    if config.enable_p2p:
        assert local_cpu_backend is not None, (
            "local_cpu_backend is required for P2PBackend"
        )
    ...
    if config.local_disk and config.max_local_disk_size > 0:
        assert local_cpu_backend is not None, (
            "local_cpu_backend is required for LocalDiskBackend"
        )
    ...
    if config.remote_url is not None:
        assert local_cpu_backend is not None, (
            "local_cpu_backend is required for RemoteBackend"
        )
    ...
    if not config.enable_pd or config.local_cpu:
        ...
        assert local_cpu_backend is not None, (
            "local_cpu_backend is required for dynamic backends"
        )
