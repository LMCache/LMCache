...
class LocalDiskBackend(StorageBackendInterface):
    ...
    def get_blocking(
        ...
        #
        # MLA fix: use multi-group shapes/dtypes if available (DeepSeek-V3 /
        # GLM-5 MLA). Without this, allocate() returns a buffer sized for the
        # first group only, and read_file silently truncates the second
        # group's bytes → on-the-wire data corruption that surfaces as
        # garbled decode tokens (not an error).
