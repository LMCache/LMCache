    ...
    def wait_for_layer(self, layer_idx: int) -> None:
        ...
        if evt is None:
            ...
            if self.result_ is None:
                ...
                # The exchange already closed: an earlier wait consumed the
                # one-shot sentinel and resolved the outcome, so no further
                # frame can arrive and blocking would burn the full timeout.
                # With load_async=False vLLM asks for every remaining layer
                # in the same forward pass, so blocking here would stall (and
                # then kill) the step before get_finished can report the
                # failed blocks for recomputation.
                self._drain_remaining()
            if _LAYERWISE_DEBUG:
                self._dbg_drain_s += time.perf_counter() - t_drain
            evt = self._layer_event_map.get(layer_idx)
        if evt is None:
            # Either the drain saw the closing sentinel or the outcome was
            # already resolved, so the exchange is over either way.
            self._resolve_final()
            if not self.result_:
