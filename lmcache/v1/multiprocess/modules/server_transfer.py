...
class PickleTransferStrategy(TransferStrategy):
    ...
    def _write_chunks(
        ...
        """
        reserved_dict = self._storage_manager.reserve_write(obj_keys, layout_desc)
        ...
        try:
            for idx, obj_key in enumerate(obj_keys):
                ...
                if memory_obj.tensor is None:
                    logger.error(
                        "pickle store reserved an object without a tensor "
                        "(chunk_index=%d, key=%s)",
                        idx,
                        obj_key,
                    )
...
class ShmTransferStrategy(TransferStrategy):
    ...
    def prepare_store(
        ...
        for gid, (g_keys, g_layout) in enumerate(
            ...
            reserved = self._storage_manager.reserve_write(g_keys, g_layout)
