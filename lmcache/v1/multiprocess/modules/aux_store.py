...
class AuxBlobStore:
    ...
    def _layout(size: int) -> MemoryLayoutDesc:
        ...
        NOTE (Jiayi): qwen35 ``reserve_write``/``submit_prefetch_task`` apply ONE
    ...
    def fetch_into_ipc(
        ...
        handle = sm.submit_prefetch_task(obj_keys, self._layout(sizes[0]))
        ...
        with sm.read_prefetched_results(obj_keys) as memory_objs:
            ...
            for i, m in enumerate(memory_objs):
                ...
                off += s
        # NOTE (Jiayi): qwen35 the copies above are ASYNC on the caller's stream. Sync
