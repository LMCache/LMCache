...
class LMCacheDrivenTransferModule(InstanceLivenessTarget):
    ...
    def release_event(self, instance_id: int, event_ipc_handle: bytes) -> None:
        ...
        entry = self.get_and_touch_context_entry(instance_id)
        if entry is not None:
            entry.cache_context.ipc_event_registry.release_exported(event_ipc_handle)
    ...
    def store(
        ...
        ipc_event_registry = cache_context.ipc_event_registry
        ...
        with (
            ...
            import_token = ipc_event_registry.hold_imported(producer_event)
            submit_callback_to_stream(
