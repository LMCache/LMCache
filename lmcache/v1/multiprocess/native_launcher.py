...
def _source_is_newer(binary: Path, source_dir: Path) -> bool:
    ...
    for path in source_dir.rglob("*"):
        ...
        if (
            ...
            or path.suffix in {".cpp", ".cu", ".cuh", ".h", ".hpp", ".txt"}
...
def ensure_native_binary(force: bool = False, *, enable_cuda: bool = False) -> Path:
    ...
    if not source_dir.exists():
        found = shutil.which(_native_mp_package_binary_name(enable_cuda))
        if found is not None:
            return Path(found)
        raise FileNotFoundError(
            "Native MP source directory is unavailable and "
            f"{_native_mp_package_binary_name(enable_cuda)} is not on PATH."
        )
...
def _validate_native_supported_args(args: argparse.Namespace) -> None:
    ...
    if chunk_size <= 0:
        raise ValueError("native MP requires positive --chunk-size")
...
def native_argv_from_args(args: argparse.Namespace) -> list[str]:
    ...
    explicit_cuda_binary = bool(os.environ.get("LMCACHE_MP_NATIVE_CUDA_BINARY"))
    enable_cuda = force_cuda or (explicit_cuda_binary and not force_no_cuda)
    ...
    if native_disk_path:
        argv.extend(["--cxx-disk-path", str(native_disk_path)])
...
def run_native_server(args: argparse.Namespace) -> None:
    ...
    env["LMCACHE_NATIVE_VERSION"] = str(utils.VERSION or "")
    env["LMCACHE_NATIVE_COMMIT_ID"] = str(utils.COMMIT_ID or "")
