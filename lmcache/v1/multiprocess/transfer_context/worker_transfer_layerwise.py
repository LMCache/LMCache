...
class LMCacheLayerwiseTransferContext(LMCacheDrivenTransferContext):
    ...
    def __init__(self, instance_id: int, req_client: RequestClient) -> None:
        """Bind the layer-wise context to one worker.
        ...
        # The per-chunk methods this path also uses are on every client, so a
        # transport without the layer-wise ones would get this far and only
        # fail once a retrieve returned None. Probe for them here, while the
        # message can still name the flag that caused it. Probed rather than
        # isinstance-checked so that duck-typed clients still work.
        missing = [
            name
            for name in ("register_layerwise_ipc_event_pool", "retrieve_layerwise")
            if not callable(getattr(req_client, name, None))
        ]
        if missing:
            raise TypeError(
                f"{type(req_client).__name__} does not implement the layer-wise "
                f"retrieve protocol ({', '.join(missing)} missing), which only "
                "the ZMQ transport serves. Start this node with --transport "
                "zmq, or without --layerwise-batch to use the per-chunk path."
            )
        self._event_pool: EventPool | None = None
        self._layerwise_batch: int = 0

    @property
    def _layerwise_client(self) -> LayerwiseRequestClient:
        """``_req_client`` narrowed to the layer-wise protocol.

        A property rather than a second attribute so the client still has one
        source of truth: the base class owns ``_req_client``, and this only
        restates the type that ``__init__`` already checked for.
        """
        return cast(LayerwiseRequestClient, self._req_client)
    ...
    def register(
        ...
        future = self._layerwise_client.register_layerwise_ipc_event_pool(
            self._instance_id
        )
        try:
    ...
    def submit_retrieve(
        ...
        self._layerwise_client.retrieve_layerwise(
            key,
            self._instance_id,
            block_ids,
            event_ipc_handle,
