...
def classify_memory_pressure(
    *,
    ...
    means reported usage has reached physical capacity. When capacity is known
    but no global watermark is configured, below-capacity usage is ``normal``;
    callers still receive the raw ratio to apply their own policy.

    Args:
        used_bytes: Current allocation, or ``None`` when collection failed.
        capacity_bytes: Physical capacity, or ``None`` for unknown/unlimited.
