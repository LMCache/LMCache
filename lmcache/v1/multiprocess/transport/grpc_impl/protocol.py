...
# First Party
from lmcache.v1.multiprocess.protocol import InstanceID, KeyType
...
def coerce_rpc_method(rpc_method: RpcMethod | str) -> RpcMethod:
    """Resolve a protobuf RPC method token."""
    if isinstance(rpc_method, RpcMethod):
        return rpc_method
    method = RpcMethod._by_method_name.get(rpc_method)
...
def get_payload_classes(rpc_method: RpcMethod | str) -> list[Any]:
    ...
    Args:
        rpc_method: The protobuf RPC method to look up.
...
def get_response_class(rpc_method: RpcMethod | str) -> Any:
    """
