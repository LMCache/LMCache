...
# Limit each client's turn so all clients and inbound replies make progress.
            ...
            if socks.get(notifier_fd) and socks[notifier_fd] & zmq.POLLIN:
                ...
                for client in self._socket_to_client.values():
                    try:
                        ...
                        logger.exception(
                            "process_outbound_task failed; continuing shared poll loop"
                        )
...
class MessageQueueClient:
    ...
    def process_outbound_task(self, max_batch: int = _OUTBOUND_BATCH_SIZE) -> bool:
        """Send at most ``max_batch`` requests and report remaining work."""
        if max_batch <= 0:
        ...
        for _ in range(max_batch):
            ...
            try:
                ...
                if len(payload_classes) != len(wrapped_request.request_payloads):
                    ...
                    raise ValueError(
                        f"Payload count mismatch for request {rpc_spec.operation}: "
                        f"expected {len(payload_classes)} payloads {expected_classes}, "
                        f"got {len(wrapped_request.request_payloads)} payloads {actual_classes}. "
                        "This is likely caused by a version mismatch between the "
                        "lmcache client and lmcache server."
                    )
                ...
                self.socket.send_multipart([b_request_uid, b_operation] + b_payloads)
