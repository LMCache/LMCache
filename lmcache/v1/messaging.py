...
class MessagePublisher(ABC):
    """Lightweight publisher for message batches with data parallelism support.

    In data parallel setups, each DP rank runs its own MessagePublisher instance
    to avoid duplicate messages and ensure proper message attribution:

    - Each DP rank creates a separate publisher
    - Publishers automatically annotate messages with their data_parallel_rank
    - This allows consumers to distinguish messages from different DP ranks

    The publisher is responsible for adding DP metadata since the client
    ...
    @abstractmethod
    def publish(self, messages: MessageBatch) -> None:
...
class ZmqMessagePublisher(MessagePublisher):
    ...
    max_queue_size:
        Maximum number of messages to buffer in memory.
    topic:
    ...
    def __init__(
        ...
        topic: str = "lmcache-messages",
    ) -> None:
        # Storage
        super().__init__(data_parallel_rank)
        self._message_queue = Queue[MessageBatch | None](maxsize=max_queue_size)
    ...
    def publish(self, messages: MessageBatch) -> None:
        ...
        if messages.data_parallel_rank is None:
            messages.data_parallel_rank = self._data_parallel_rank
        self._message_queue.put(messages)
        logger.debug("Added message to queue for publishing")

    def shutdown(self) -> None:
        """Stop the publisher thread and clean up resources."""

        logger.info("Shutting down the message publisher")
        self._running = False
        ...
        while pending_items and (time.time() - start < self.SHUTDOWN_TIMEOUT):
            pending_items = not self._message_queue.empty()
            if pending_items:
                time.sleep(0.1)
    ...
    def _socket_setup(self) -> None:
        ...
        # 2) lets us send back one request → many replies (streamed messages)
    ...
    def _publisher_thread(self) -> None:
        ...
        while self._running or self._message_queue.qsize() > 0:
            ...
            try:
                ...
                logger.debug(
                    f"Publishing message '{message}' in sequence '{seq}' "
                    f"to topic '{self._topic_bytes.decode('utf-8')}'"
                )
                payload = self._pack.encode(message)
...
class MessagePublisherFactory:
    _registry: dict[str, Callable[..., MessagePublisher]] = {
        "null": NullMessagePublisher,
        "zmq": ZmqMessagePublisher,
    }

    @classmethod
    def register_publisher(
        cls, name: str, ctor: Callable[..., MessagePublisher]
    ) -> None:
    ...
    def create(
        ...
        max_queue_size: int = 100_000,
        topic: str = "lmcache-messages",
        message_publisher: str = "zmq",
    ) -> MessagePublisher:
        """Create publisher from a config mapping."""
        try:
            constructor = cls._registry[message_publisher]
        except KeyError as exc:
            raise ValueError(
                f"Unknown message publisher '{message_publisher}'"
            ) from exc
        return constructor(
