...
class SGLangGPUConnector(GPUConnectorInterface):
    """
    The GPU KV cache should be a list of tensors, one for each layer, with separate key and value pointers.
    More specifically, we have:
    ...
    The connector manages the transfer of KV cache data between CPU and GPU memory for SGLang using pointer arrays for efficient access.
    It will produce/consume memory objects with KV_2LTD format.

    Usage notes:
    - The `to_gpu` and `from_gpu` methods expect 'kvcaches' and 'slot_mapping' in kwargs.
    - The slot_mapping should be a partial slot mapping, with length equal to the uncached token sequence.
    - In the case of prefix caching, slot_mapping will start with -1s until the end of the matched prefix. The start and end indices should never overlap with the prefix caching (the underlying CUDA kernel should never see -1 in slot_mapping).
    - The connector maintains key and value pointer arrays on both CPU and GPU for each device index.
    - Optionally, a GPU buffer can be used for intermediate transfers if use_gpu is True.
    """

    def __init__(self, hidden_dim_size: int, num_layers: int, use_gpu: bool = False, **kwargs):
        self.hidden_dim_size = hidden_dim_size
        self.num_layers = num_layers
        self.key_pointers = torch.empty(
            num_layers, dtype=torch.int64, device="cpu"
        )
        self.value_pointers = torch.empty(
            num_layers, dtype=torch.int64, device="cpu"
        )
