...
class VLLMPagedMemGPUConnectorV2(GPUConnectorInterface):
    ...
    def _initialize_pointers(self, kv_caches: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert self.device.type == accelerator.name, f"The device should be {accelerator.name}."
    ...
    def from_gpu(self, memory_obj: MemoryObj, start: int, end: int, **kwargs):
        ...
        if not memory_obj.tensor.device.type == accelerator.name:
...
class SGLangGPUConnector(GPUConnectorInterface):
    ...
    def _initialize_pointers(self, kv_caches: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert device.type == accelerator.name, f"The device should be {accelerator.name}."
        idx = device.index
