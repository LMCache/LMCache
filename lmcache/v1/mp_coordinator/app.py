...
thin, operating on the shared collaborators carried on ``app.state``:
``config``, ``registry``, and ``push_client``. The lifespan owns the shared HTTP
client and the background health-check task (eviction of instances whose
heartbeats have lapsed).

Adding a capability = a new ``http_apis/<name>_api.py`` router (auto-discovered)
that uses those shared collaborators. A domain with real logic/state of its own
adds a ``<name>_service.py`` constructed over them and stashed on ``app.state``
here; thin domains (like membership) just use the registry directly.
"""
...
from lmcache.v1.mp_coordinator.push_client import HttpPushClient
...
def create_app(config: MPCoordinatorConfig) -> FastAPI:
    ...
    Returns:
        ...
        collaborators (``config``, ``registry``, ``push_client``); all
        ``http_apis`` routers are registered.
    """
    registry = InstanceRegistry()
    http_client = httpx.AsyncClient()
    push_client = HttpPushClient(registry, http_client)
    ...
    app.state.push_client = push_client
