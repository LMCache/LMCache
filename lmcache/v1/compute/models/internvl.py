...
from lmcache.v1.compute.models.utils import patch_vllm_dummy_video_inputs


class LMCInternVLModel(LMCBaseModel):
    """
    LMCache Q/K/V preprocessing for InternVL family (incl. InternVL3-2B).
    - Most InternVL variants do NOT use per-head q_norm / k_norm like Qwen3.
    ...
    def __init__(self, vllm_model, blender, enable_sparse: bool = False):
        # Patch vLLM dummy video generator so profiling produces uint8 frames.
    ...
    def _process_qkv(self, q, k, v, layer):
        ...
        # If Qwen2.5 variant defines q_norm/k_norm, apply them per-head.
