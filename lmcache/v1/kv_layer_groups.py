...
class KVLayerGroupsManager:
    ...
    def _detect_object_groups(
        ...
        null block zero. Separated groups bucket by recurrent policy, sliding
        ...
        Notes:
            ``separate_object_groups=False`` is a preference, not a guarantee.
            A group declaring a non-default ``null_block_id`` is sparse: its
            chunks may legitimately hold no data. Sharing one object with a
            dense group would make object presence ambiguous -- a retrieve
            could not tell "this group stored nothing here" from "this object
            is missing". Separation is therefore forced for soundness, and
            logged, because it changes the object-group ids that feed the
            object key namespace.
        """
        sparse_groups = [
            idx
            for idx, group in enumerate(self._kernel_groups)
            if group.null_block_id != 0
        ]
        if sparse_groups and not self._separate_object_groups:
            logger.info(
                "Forcing object-group separation despite "
                "separate_object_groups=False: kernel groups %s declare a "
                "non-default null_block_id and cannot share object presence "
                "with dense groups.",
                sparse_groups,
            )
        if not self._separate_object_groups and not sparse_groups:
            return [
                ObjectGroupInfo(
