...
# ``_version.py`` is generated by setuptools_scm at build time and may
# be missing in dev checkouts that have not been built.  Resolve once
# at import time so callers do not pay the import cost per recorder.
try:
    # First Party
    from lmcache._version import (
        __version__ as _LMCACHE_VERSION,  # type: ignore[import-not-found]
    )
except ImportError:
    ...
    * the trace-gate flip (on at construction, off at ``close()``)
    ...
    def _write_header(self, sm_config_json: str, sm_config_digest: str) -> None:
        header = Header(
            ...
            level=self._level,
            lmcache_version=str(_LMCACHE_VERSION),
