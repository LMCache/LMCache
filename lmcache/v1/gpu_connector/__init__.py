...
def CreateGPUConnector(
    ...
    if engine == EngineType.SGLANG:
        ...
        xpu_dev = getattr(torch, "xpu", None)
        if xpu_dev is not None and xpu_dev.is_available():
            dev_name = "xpu"
            xpu_dev.set_device(local_worker_id)
        elif torch.cuda.is_available():
            dev_name = "cuda"
            torch.cuda.set_device(local_worker_id)
        else:
            raise RuntimeError("No supported device found for SGLang connector.")
