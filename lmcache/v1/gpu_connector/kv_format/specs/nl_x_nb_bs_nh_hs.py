...
"""Independent SGLang components: ``NL x [NB, BS, NH, HS]``.

Each list entry is one physical K or V allocation. Consequently, ``NL`` is
the number of registered component tensors, not the model's logical layer
count; no K/V pairing or list restructuring is required.
"""
...
class NL_X_NB_BS_NH_HS_Spec(KVFormatSpec):
    engine_kv_format = lmcache_native.EngineKVFormat.NL_X_NB_BS_NH_HS
    attention_backends = ("SGLang unified MP independent components",)
    is_layer_list = True
    is_single_kv = True
    ...
    def hidden_dim(self, layer_idx: int = 0) -> int:
        tensor = self.kv_caches[layer_idx]
