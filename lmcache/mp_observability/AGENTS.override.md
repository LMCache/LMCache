...
When working in `lmcache/mp_observability/`:

Design docs live in `docs/design/mp_observability/` — update them whenever
the contracts below change.

1. **New `EventType`** — after adding an entry to `event.py`, update the
   metadata contract table in `docs/design/mp_observability/EVENTS.md`
   with the new type's metadata keys and types.

2. **New metrics subscriber** — after adding counters/histograms, update the
   metrics table in `docs/design/mp_observability/METRICS.md` with the
...
3. **New subscriber class** — follow the step-by-step guide in `README.md`
   ...
   the design rules in `docs/design/mp_observability/event-bus.md`.
