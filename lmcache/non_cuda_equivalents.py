...
class PageBufferShapeDesc:
    ...
    without the compiled extension.  ``dtype`` is a Python-only extension
    used by the pure-Python kernel fallback to disambiguate float16 vs.
    bfloat16 (both have ``element_size == 2``); it has no C++ counterpart.
    """
