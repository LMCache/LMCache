...
Usage examples::
    ...
    lmcache test-cache --url tcp://localhost:5555 \\
...
def _send_request(
    ...
    if sock.poll(timeout_ms, zmq.POLLIN):
        ...
        return resp[2:] if len(resp) >= 3 else None
    return None
...
def _process_request(
    ...
    # Wrap with modulo to stay within allocated capacity.
    num_blocks = num_full_tokens // block_size
    block_offset = (seq_no * num_blocks) % total_blocks
