...
class _SubpagedAttentionViewEdit(KVCacheGroupEdit):
    ...
    Both registered attention layouts are handled: the rank-5 K/V-split
    layout of vLLM <= 0.25.x and the rank-4 K/V-fused layout of vLLM >= 0.26
    (an int8 byte-view since 0.30). The re-view is a byte-range
    ...
    def matches(self, spec: KVCacheSpec, kv_cache: RegisteredKVCache) -> bool:
        return (
            ...
            # A paged attention layout -- rank 5 (num_blocks, 2, block_size,
            # num_heads, head_size) through vLLM 0.25.x, rank 4 (num_blocks,
            # num_heads, block_size, content) since 0.26 -- whose block dim
            # disagrees with the scheduler block-id unit: the backend re-paged
            # the tensor at its kernel block size.
            and isinstance(kv_cache, torch.Tensor)
            and kv_cache.ndim in (4, 5)
    ...
    def apply(
        ...
        The tensor is kernel-paged, rank 5 ``(num_kernel_pages, 2,
        kernel_block_size, num_kv_heads, head_size)`` or rank 4
        ``(num_kernel_pages, num_kv_heads, kernel_block_size, content)``;
        the result is ``(num_logical_blocks, 2, spec.block_size, num_heads,
        head_size)`` over the same storage. A rank-4 tensor is a permute view
        of the backend's physical layout, so it is re-viewed in memory order;
        a rank-5 tensor must already be contiguous in logical order.
        ...
        if (
            ...
            or (kv_cache.ndim == 5 and kv_cache.shape[1] != 2)
        ):
            got = (
            ...
            raise ValueError(
                ...
                f"or (num_blocks, num_heads, block_size, content) "
        ...
        if kv_cache.ndim == 4:
            # The fused rank-4 layout is registered as a permute view of the
            # backend's physical layout (under NHD a transpose of dims 1, 2);
            # kernel pages tile the logical page by byte range only in memory
            # order, so re-view with dims in descending stride order.
            kv_cache = kv_cache.permute(
                sorted(range(4), key=kv_cache.stride, reverse=True)
            )
            if kv_cache.shape[0] != num_kernel_pages:
                raise ValueError(
                    f"expected a num-blocks-first KV cache layout; outermost "
                    f"memory dim is {kv_cache.shape[0]}, not the kernel page "
