...
class LMCacheMPConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def has_pending_push_work(self) -> bool:
        ...
        Returns:
            True when scheduler-side lazy offload has queued store metadata or
            submitted stores waiting for worker completion. Non-lazy mode uses
            vLLM's normal delayed-free path and does not need this keepalive.
        """
        if self.role != KVConnectorRole.SCHEDULER or not self.lazy_offload:
            return False

        pending_store = getattr(self, "_pending_store", None)
        return pending_store is not None and pending_store.has_pending_work()
