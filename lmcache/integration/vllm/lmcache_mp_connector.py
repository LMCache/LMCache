...
from lmcache.integration.vllm.kv_cache_group_edits import (
    cb_kept_group_indices,  # kept vLLM group indices
)
from lmcache.integration.vllm.kv_cache_group_edits import (
    cb_skip_mamba_groups,  # CacheBlend-on-GDN gated mamba-skip
)
from lmcache.integration.vllm.kv_cache_group_edits import (
...
class LMCacheMPConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def __init__(
        ...
        # CacheBlend-on-GDN (gated) — serve only full-attention
    ...
    def update_state_after_alloc(
        ...
        # project vLLM's full-group block ids onto the compacted,
    ...
    def _process_cached_requests(
        ...
        for idx, request_id in enumerate(cached_reqs.req_ids):
            ...
            # Update block ids
            # project onto compacted (mamba-skipped) group order.
    ...
    def _report_block_allocation_deltas(
        ...
        for idx, request_id in enumerate(cached_reqs.req_ids):
            ...
            # project so "group 0" is the kept primary (full-attn),
