...
class LMCacheKvConnectorScheduler(KvCacheConnectorScheduler):
    """Scheduler that queries the LMCache engine for cached token counts
    and builds per-request load/save metadata for the worker."""
    ...
    def __init__(self, llm_args: TorchLlmArgs) -> None:
        ...
        self._pending: dict = {}
        # lazy: engine is created by the worker in register_kv_caches,
        # which runs concurrently with scheduler __init__
    ...
    def get_num_new_matched_tokens(
        ...
        what TRT-LLM already matched via GPU block reuse.
        ...
        Returns:
            ``(new_matched, is_async)``: extra tokens LMCache can load
            from CPU/remote cache, and whether the load will be async
            (always ``False`` in this adapter).
        """
        t0 = time.perf_counter()

        # guard: race if scheduler runs before worker finishes register_kv_caches
        if not self._engine:
        ...
        # guard: trt-llm should always pass block-aligned positions
    ...
    def build_connector_meta(
        ...
        """Build per-request load/save specs from the pending lookup results.

        Called by the runtime after scheduling.  The returned metadata is
        bound to the worker via ``bind_connector_meta`` before the forward
        pass begins.
        """
    ...
    def request_finished(self, request: LlmRequest, cache_block_ids: List[int]) -> bool:
        ...
        Returns whether async saving is in progress (always ``False`` —
        saves are synchronous in this adapter).
        """
        return False
    ...
    def update_state_after_alloc(
        ...
        """Called after block allocation to provide block IDs.

        No-op: block IDs are captured in ``build_connector_meta`` via
        ``scheduler_output.new_requests``.
        """
        pass


class LMCacheKvConnectorWorker(KvCacheConnectorWorker):
    """Worker that performs GPU↔CPU KV cache transfers via the LMCache engine."""
    ...
    def start_load_kv(self, stream: torch.cuda.Stream) -> None:
        ...
        forward-pass stream.  Cross-layer format loads all layers in a
        single kernel — no per-layer overlap to exploit.
        """
        meta = self._meta
    ...
    def wait_for_layer_load(self, layer_idx: int, stream: torch.cuda.Stream) -> None:
        """No-op: cross-layer format loads all layers at once in start_load_kv."""
        pass

    def save_kv_layer(self, layer_idx: int, stream: torch.cuda.Stream) -> None:
        """Per-layer save hook.  No-op: saves are batched in ``wait_for_save``."""
        pass

    def wait_for_save(self, stream: torch.cuda.Stream) -> None:
        """Store newly computed KV blocks from GPU to LMCache's CPU cache.

        Waits for the forward-pass stream, then calls ``engine.store``
        for each request with new blocks.  Synchronizes the store stream
        before returning.
        """
        meta = self._meta
    ...
    def get_finished(
        ...
        """Return request IDs that have finished saving and loading.

        All operations are synchronous in this adapter, so nothing is
        ever pending — returns empty lists.
        """
        return [], []
