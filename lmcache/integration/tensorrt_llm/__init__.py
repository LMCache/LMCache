...
**In-process mode** (``lmcache``)
    The LMCache engine runs inside the TRT-LLM process.  Simple to set up,
...
**Multi-process mode** (``lmcache-mp``)
    The LMCache engine runs as a standalone ZMQ server, providing process
    isolation and shared KV caching across multiple TRT-LLM instances on
    the same node.
...
Both modes can be configured explicitly instead of using presets::
