...
Implements LMCacheMPKvConnectorScheduler and LMCacheMPKvConnectorWorker,
the two classes required by TRT-LLM's kv_connector_config.

Unlike the in-process adapter (tensorrt_adapter.py), this adapter
communicates with a standalone LMCache server over ZMQ, allowing
process isolation and shared caching across multiple TRT-LLM instances.
"""
...
def _get_server_url(llm_args: "TorchLlmArgs") -> str:
    """Resolve server URL: config field > env var > default."""
...
class LMCacheMPKvConnectorScheduler(KvCacheConnectorScheduler):
    ...
    def __init__(self, llm_args: TorchLlmArgs) -> None:
        ...
        # request_id -> (all_tokens, num_matched): lookup results from
        # get_num_new_matched_tokens, consumed by build_connector_meta
        self._pending: dict = {}

        # ZMQ client for communicating with LMCache server
    ...
    def get_num_new_matched_tokens(
        ...
        Sends a LOOKUP request to the MP server and polls until the
        prefetch completes.  See the in-process adapter for the full
        ...
        try:
            # submit LOOKUP, query status once; if prefetch hasn't finished
            # we return 0 (no poll loop — the scheduler call must be fast)
            future = _send_request(self._mq_client, RequestType.LOOKUP, [key, 1])
        ...
        # LOOKUP acquired read locks on chunks in [0, cached_tokens).
        # trt-llm already has [0, num_computed_tokens), so those chunks
        # will never be retrieved — free their locks (chunk-aligned) to
        # avoid holding them until TTL expiry.
        ...
        if overlap_end > 0:
            free_key = self._create_key(
                all_tokens, start=0, end=overlap_end, request_id=request.request_id
            ).no_worker_id_version()
            try:
                _send_request(
                    self._mq_client,
    ...
    def request_finished(self, request: LlmRequest, cache_block_ids: List[int]) -> bool:
        ...
        Saves are synchronous, so we never need to defer deallocation —
        return False. But we still call END_SESSION to release the
        server-side token-hash/session state for this request.
        """
        try:
            _send_request(
                self._mq_client,
    ...
    def update_state_after_alloc(
        ...
        """No-op: block IDs captured in ``build_connector_meta``."""
        pass
...
class LMCacheMPKvConnectorWorker(KvCacheConnectorWorker):
    ...
    def register_kv_caches(self, kv_cache_tensor: torch.Tensor) -> None:
        """Register the KV cache tensor with the LMCache server via CUDA IPC.

        TRT-LLM provides a 4D tensor [NB, NL, 2, NH*BS*HS]. We reshape it
        to 6D [NB, NL, 2, NH, BS, HS] so the server's GPUCacheContext can
        parse it using the NB_NL_TWO_NH_BS_HS format.
        """
        if self._registered:
            logger.info("LMCache MP worker: KV caches already registered")
            return

        # Derive NH, BS, HS from model config
        # Third Party
        ...
        # Share the tensor via direct cudaIpcGetMemHandle (bypasses PyTorch's
        # _share_cuda_() which fails on TRT-LLM's at::for_blob tensors).
        # Pass reshape info in layout_hints so the server can view it as 6D.
    ...
    def start_load_kv(self, stream: torch.cuda.Stream) -> None:
        """Send RETRIEVE requests to the MP server for each pending load."""
    ...
    def save_kv_layer(self, layer_idx: int, stream: torch.cuda.Stream) -> None:
        """No-op: saves are batched in ``wait_for_save``."""
        pass

    def wait_for_save(self, stream: torch.cuda.Stream) -> None:
        """Send STORE requests to the MP server for each pending save."""
        meta: Optional[LMCacheMPConnectorMetadata] = self._metadata
