...
class LMCacheMPKvConnectorWorker(KvCacheConnectorWorker):
    ...
    def _create_and_record_event(self, stream: object) -> object:
        ...
        record_event(event, self._device, stream)
        return event
            ...
            try:
                success = (
                    self._req_client.retrieve(
                        ...
                        self._instance_id,
                        [spec.block_ids],
                        export_event(event, self._device),
                ...
                raise RuntimeError(
                    f"LMCache MP worker retrieve failed for request {req_id}"
                ) from e

            if not success:
                raise RuntimeError(
                    "LMCache MP worker retrieve returned False for request "
