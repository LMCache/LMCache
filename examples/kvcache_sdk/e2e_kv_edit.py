...
def _source_and_target_tokens(
    ...
    *,
    ...
    suffix_tokens = cache_tokens - fake_prefix_tokens
    common_suffix = _suffix_with_min_tokens(tokenizer, SOURCE_PARAGRAPH, suffix_tokens)
...
def run(args: argparse.Namespace, ctx: lmc_sdk.LMCacheKVCacheContext) -> None:
    ...
    if source_tokens[args.fake_prefix_tokens:] != target_tokens[args.fake_prefix_tokens:]:
        raise RuntimeError(
            "source and target prompts must match after the fake prefix"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--lmcache-model-name",
        default="",
        help="Model name registered in LMCache MP server (default: same as vllm-model-name).",
    )
    ...
    parser.add_argument("--mq-url", required=True)
...
def main() -> None:
    ...
    ctx = lmc_sdk.connect(
        ...
        model_name=args.lmcache_model_name or args.vllm_model_name,
        timeout=args.timeout,
    )
