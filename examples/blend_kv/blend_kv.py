...
from lmcache_vllm.blend_adapter import combine_input_prompt_chunks, append_separator
import time
import torch
...
def precompute_kv(text_chunk, llm):
    sampling_params_prefix = SamplingParams(temperature=0.0, top_p=0.95, max_tokens=1)
...
llm = LLM(model="mistralai/Mistral-7B-Instruct-v0.2", gpu_memory_utilization=0.5, tensor_parallel_size=1)
sampling_params_generation = SamplingParams(temperature=0.0, top_p=0.95, max_tokens=30)


print("-------------------- Pre-computing KV cache for the chunks -------------------------")
for chunk in chunks:
