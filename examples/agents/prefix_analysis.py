...
DEFAULT_POOL_SIZES_GB = [1, 2, 4, 8, 16, 32, 50, 100, 200, 500, "unlimited"]
...
def plot_hit_rates(
    ...
    plt.plot(range(len(hit_rates)), hit_rates, marker='o', linewidth=2, markersize=8, color='#2E86AB')
    ...
    for i, (rate, label) in enumerate(zip(hit_rates, x_labels)):
        plt.annotate(f'{rate*100:.1f}%', 
                    xy=(i, rate), 
                    xytext=(0, 8), 
...
Examples:
  ...
  %(prog)s -i trace.jsonl --pool-sizes 1 2 4 8 16 unlimited
    ...
    parser.add_argument(
        ...
        help=f'Conversion factor from GB to tokens (default: {DEFAULT_TOKENS_PER_GB}). '
             'This varies by tokenizer and should be adjusted when using a different tokenizer.'
    )
...
def parse_pool_sizes(pool_sizes_input: Optional[List[str]]) -> List:
    ...
    for size in pool_sizes_input:
        if size.lower() == "unlimited":
            ...
            try:
                ...
                raise ValueError(f"Invalid pool size: {size}. Must be a number or 'unlimited'")
