...
def benchmark(args):
    ...
    token_pool = [
        random_token + i for i in range(args.seq_num)
    ]
    ...
    for i in range(args.seq_num):
        success, hash_prefix = lmcache_store_kv(engine, token_pool[i], kv_cache[:, :, indices[i]])
        if not success:
    ...
    start_time = time.time()
    for i in range(args.seq_num):
        success = lmcache_retrieve_kv_hash(engine, hash_pool[i],
                                 kv_cache[:, :, indices[i]])
        if not success:
            print(
    ...
    print(f"store throughput: {kvsize * args.seq_num / (store_latency / 1000) / 1e6} MB/s")
