...
def test():
    ...
    # Generate a kv cache pool same as hierarchical kv cache
    ...
    store_status = torch.full((seq_len // engine.config.chunk_size, ),
                              StoreStatus.FAIL,
                              dtype=torch.int32)
    hash_keys = lmcache_store_kv(engine, tokens, kv_cache[:, :, indices],
                                 store_status)
    print("Failure in storing kv cache in LMCache:",
          torch.any(store_status == StoreStatus.FAIL))
    ...
    suffix_hash_keys = lmcache_store_kv_hash(engine, suffix_hash,
                                             kv_cache[:, :, suffix_indices],
                                             store_status)
    # suffix_hash_keys = lmcache_store_kv(engine, suffix_tokens,
    # kv_cache[:, :, suffix_indices], store_status, prefix_hash=prefix_hash)
    print("Failure in storing kv cache in LMCache:",
          torch.any(store_status == StoreStatus.FAIL))
