...
from lmcache.integration.sglang.sglang_adapter import init_lmcache_engine, lmcache_store_kv, lmcache_retrieve_kv, get_hash, lmcache_retrieve_kv_hash, lmcache_store_kv_hash, StoreStatus, RetrieveStatus
...
def test():
    ...
    store_status = torch.full((seq_len // engine.config.chunk_size,), StoreStatus.FAIL, dtype=torch.int32)
    hash_keys = lmcache_store_kv(engine, tokens, kv_cache[:, :, indices], store_status)
    print("Failure in stroing kv cache in LMCache:", torch.any(store_status == StoreStatus.FAIL))
    ...
    retrieve_status = torch.full((seq_len // engine.config.chunk_size,), RetrieveStatus.FAIL, dtype=torch.int32)
    lmcache_retrieve_kv(engine, tokens, buffer, retrieve_status)
    print("Failure in retrieving kv cache from LMCache:", torch.any(retrieve_status == RetrieveStatus.FAIL))
    ...
    print("Failure in retrieving kv cache from LMCache using hash:", torch.any(retrieve_status == RetrieveStatus.FAIL))
    ...
    suffix_hash_keys = lmcache_store_kv_hash(engine, suffix_hash, kv_cache[:, :, suffix_indices], store_status)
    # suffix_hash_keys = lmcache_store_kv(engine, suffix_tokens, kv_cache[:, :, suffix_indices], store_status, prefix_hash=prefix_hash)
    print("Failure in stroing kv cache in LMCache:", torch.any(store_status == StoreStatus.FAIL))
    ...
    buffer = torch.zeros(2, model_config.num_hidden_layers, seq_len, model_config.get_num_kv_heads(tensor_parallel_size), model_config.head_dim)
    retrieve_status = torch.full((seq_len // engine.config.chunk_size,), RetrieveStatus.FAIL, dtype=torch.int32)
    lmcache_retrieve_kv(engine, suffix_tokens, buffer, retrieve_status, prefix_hash)
    print("Failure in retrieving kv cache from LMCache:", torch.any(retrieve_status == RetrieveStatus.FAIL))
    ...
    buffer = torch.zeros(2, model_config.num_hidden_layers, tokens.shape[0], model_config.get_num_kv_heads(tensor_parallel_size), model_config.head_dim)
    retrieve_status = torch.full((tokens.shape[0] // engine.config.chunk_size,), RetrieveStatus.FAIL, dtype=torch.int32)
    lmcache_retrieve_kv(engine, tokens, buffer, retrieve_status)
    print("Failure in retrieving kv cache from LMCache:", torch.any(retrieve_status == RetrieveStatus.FAIL))
    ...
    # Assert the retrieved kv cache is close to the stored one
    if torch.allclose(buffer, kv_cache[:, :, indices], atol=1e-1):
        print("Overall KV cache retrieved successfully matches the stored one.")
    else:
