...
def test():
    ...
    load_status = torch.full((seq_len // engine.config.chunk_size,), StoreStatus.FAIL, dtype=torch.int32)
    hash_keys = lmcache_store_kv(engine, tokens, kv_cache[:, :, indices], load_status)
    print("Failure in stroing kv cache in LMCache:", torch.any(load_status == StoreStatus.FAIL))
    ...
    suffix_hash_keys = lmcache_store_kv(engine, suffix_tokens, kv_cache[:, :, suffix_indices], load_status, prefix_hash)
    print("Failure in stroing kv cache in LMCache:", torch.any(load_status == StoreStatus.FAIL))
