...
2. Send a request with caching enabled (default behavior):
...
Note that cache is stored by default. To disable caching, pass `"kv_transfer_params": {"store_cache": false}` in your request. This works for both `/v1/completions` and `/v1/chat/completions` endpoints.
