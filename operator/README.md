...
> [!WARNING]
> **Upgrade note:** on NVIDIA (`gpuVendor: nvidia`, the default) the operator now defaults to **isolated IPC** (`spec.isolatedIPC` unset resolves to `true`): the engine runs with `--isolated-ipc`, KV caches and completion events travel over driver-level CUDA IPC, and **no** `hostIPC` or `/dev/shm` sharing is wired at all. This requires an LMCache image that understands `--isolated-ipc`. Set `spec.isolatedIPC: false` to keep the legacy `/dev/shm`-sharing wiring — you need that for older images, and for allocators backed by the CUDA VMM API (vLLM sleep mode / `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`), which driver-level CUDA IPC cannot share. Upgrading restarts the engine pods once (the DaemonSet pod template changes); restart vLLM pods afterwards so the webhook re-injects them with the matching connector config.

> [!WARNING]
> **Upgrade note:** earlier operator versions always set `hostIPC: true` on engine pods and webhook-injected vLLM pods. `spec.hostIPC` now defaults to `false`, and applies only when `spec.isolatedIPC` resolves to `false` (isolated IPC needs neither). Under `isolatedIPC: false` the host's `/dev/shm` is shared via a hostPath mount instead (the CUDA IPC requirement); a pod using `hostIPC` and a pod using the hostPath mount see the same host `/dev/shm`. If your cluster blocks hostPath volumes, set `spec.hostIPC: true` (or leave isolated IPC on).
...
The chart installs the operator, its three CRDs, RBAC, webhook, and certificate resources. Create engine and coordinator custom resources separately; the chart does not create cache workloads. `--create-namespace` creates the operator namespace without making it part of the Helm release. The default release name preserves the existing `lmcache-operator-*` object names.

**Option B: Rendered YAML from a release**

The same chart also produces `install.yaml`, which includes the namespace. cert-manager is required for this installation too:
...
`make deploy` uses the local chart with `helm upgrade --install`. Use [chart values](charts/lmcache-operator/values.yaml) to configure the operator, including its image, resources, scheduling, and optional ServiceMonitor. Cache server settings belong in the engine custom resources.

If you enable `metrics.serviceMonitor.enabled`, bind the `<release>-metrics-reader` ClusterRole to your Prometheus ServiceAccount so it can scrape the operator's authenticated HTTPS metrics. Configure Prometheus to discover the ServiceMonitor's namespace and labels as well.
...
The CRDs carry `helm.sh/resource-policy: keep`, so uninstall retains the three CRDs, their custom resources, and the cache workloads they own. The namespace also remains. Reconciliation and admission injection stop until the operator is installed again. `make uninstall` is a separate, destructive CRD cleanup: it deletes all instances of these custom resources across the cluster and can remove their owned workloads. Use it only when removing the caches themselves.

An existing `kubectl apply` installation is **not adopted automatically**. To move it to Helm, use the same release name and namespace and first compare the chart's rendered resources with the installed objects:

```bash
# Run from the repository root, using the chart version you intend to install.
...
After reviewing the diff and confirming that these objects belong to this operator installation, explicitly transfer ownership with Helm 3.17+:
...
Pass the same values file to both commands if you customize the chart. Do not delete the existing `install.yaml` resources to migrate: that would delete the CRDs and namespace, including workloads you intend to retain. Once adopted, manage the operator through Helm instead of applying the old installer.
...
- **IPC wiring must match the engine's mode** — under the default isolated IPC, nothing is shared: the handles rendezvous in the kernel driver, and the webhook gives the pod a private memory-backed `/dev/shm` for vLLM's own workers. With `spec.isolatedIPC: false`, CUDA IPC instead needs both pods to see the same `/dev/shm` tmpfs (PyTorch's CUDA IPC handles reference a shared-memory ref-counter file there): mount the host's `/dev/shm` via hostPath on the vLLM pod (the injection webhook does this for you), or set `spec.hostIPC: true` on the engine to use the host IPC namespace instead. In that legacy mode, GPU memory mapping fails without a shared `/dev/shm`.
...
> [!WARNING]
> With `spec.isolatedIPC: false`, **do NOT mount an emptyDir at `/dev/shm`** on either LMCache or vLLM pods. In that legacy mode both pods must share the host's `/dev/shm` (via the default hostPath mount, or via `hostIPC: true`); mounting an emptyDir (even with `medium: Memory`) shadows it with a private tmpfs, breaking CUDA IPC — `cudaIpcOpenMemHandle` fails because IPC handles from one pod become invisible to the other. Under the default isolated IPC this constraint disappears: a private `/dev/shm` is exactly what the pods get.
...
- `--kv-transfer-config <JSON>` — the `LMCacheMPConnector` config, inlined onto
  the vLLM container args (no volume mount needed); under isolated IPC it
  carries `lmcache.mp.isolated_ipc: true` so the connector picks the
...
- the `/dev/shm` wiring matching the engine's IPC mode — a pod-private
  ...
  need more than Kubernetes' 64Mi default), a hostPath mount of the host's
  `/dev/shm` when `spec.isolatedIPC: false` (or `hostIPC: true` instead, when
  the engine sets `spec.hostIPC: true`);
...
> rejected by `baseline`/`restricted`); the default isolated IPC injects only
> an emptyDir, which those profiles allow. The engine must
