...
type LMCacheEngineSpec struct {
	...
	// backends are NVIDIA-only). When it resolves to true it takes priority
	// over hostIPC and the /dev/shm sharing: the engine pod gets neither, the
	// server is started with --isolated-ipc, the connection ConfigMap carries
	// lmcache.mp.isolated_ipc, and the injection webhook gives opted-in vLLM
	// pods a pod-private memory-backed /dev/shm instead of the host mount.
	// Requires an LMCache image that understands --isolated-ipc; set to false
	// to keep the legacy /dev/shm-sharing wiring (e.g. for older images, or
	// for allocators backed by the CUDA VMM API, which driver-level CUDA IPC
	// cannot share — vLLM sleep mode / expandable segments).
	// +optional
