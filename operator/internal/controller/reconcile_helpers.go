...
// master-key secret exists in the engine's namespace. It mirrors
// reconcileRESPAuthSecret: a no-op unless spec.l2Backend.serde.aesgcm is set,
// otherwise it reads the user-created source secret, validates it, and
// creates/updates a managed copy owned by the engine. The reference is
...
func (r *LMCacheEngineReconciler) reconcileL2EncryptionSecret(ctx context.Context, engine *lmcachev1alpha1.LMCacheEngine) error {
	log := logf.FromContext(ctx)
	spec := &engine.Spec

	// No aesgcm serde configured — clean up any stale managed secret.
...
// deleteL2EncryptionSecretIfExists removes the managed L2 encryption secret
// if it exists (e.g. when encryption is removed from the spec).
func (r *LMCacheEngineReconciler) deleteL2EncryptionSecretIfExists(ctx context.Context, engine *lmcachev1alpha1.LMCacheEngine) error {
	secret := &corev1.Secret{}
