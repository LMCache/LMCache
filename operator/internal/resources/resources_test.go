...
func TestBuildDaemonSet_RuntimeClassNameOverride(t *testing.T) {
	engine := minimalEngine()
	engine.Spec.RuntimeClassName = ptr("nvidia-cdi")
	engine.SetDefaults()

	ds := BuildDaemonSet(engine)
	podSpec := ds.Spec.Template.Spec

	if podSpec.RuntimeClassName == nil || *podSpec.RuntimeClassName != "nvidia-cdi" {
		t.Fatalf("expected RuntimeClassName=nvidia-cdi, got %v", podSpec.RuntimeClassName)
	}
	if _, ok := ds.Spec.Template.Annotations["nvidia.cdi.k8s.io/container."+engineContainerName]; ok {
		t.Fatal("explicit RuntimeClass should not add the NRI/CDI annotation")
	}
}
