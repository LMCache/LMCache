...
class UserSessionManager:
    ...
    def _load_sharegpt_data(self):
        ...
        logger.info("There are %s users satisfying ", len(self.sharegpt_data))
    ...
    def _remove_finished_sessions(self):
        ...
        if len(sessions_to_remove) > 0:
            logger.info(
                "Removing %s finished sessions, now "
                "active users: %s",
...
def process_output(filename):
    logger.warning(
        "Processing the existing summary file %s"
