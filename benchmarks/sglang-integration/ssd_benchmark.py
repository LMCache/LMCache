...
from lmcache.integration.sglang.sglang_adapter import (
    RetrieveStatus, StoreStatus, get_hash, init_lmcache_engine,
    lmcache_retrieve_kv, lmcache_retrieve_kv_hash, lmcache_store_kv)
...
def benchmark(args):
    ...
    store_status = torch.full((args.seq_len // engine.config.chunk_size, ),
                              StoreStatus.FAIL,
                              dtype=torch.int32)
    retrieve_status = torch.full((args.seq_len // engine.config.chunk_size, ),
                                 RetrieveStatus.FAIL,
                                 dtype=torch.int32)
    ...
    for i in range(args.seq_num):
        lmcache_retrieve_kv(
            engine, token_pool[i],
            buffer[:, :,
                   i * args.seq_len:(i + 1) * args.seq_len], retrieve_status)
        if torch.any(retrieve_status == RetrieveStatus.FAIL):
            print(f"Failure in retrieving KV cache for sequence {i}")
        retrieve_status.fill_(
            RetrieveStatus.FAIL)  # Reset status for next iteration
    ...
    # check correctness
    for i in range(args.seq_num):
        if not torch.allclose(buffer[:, :,
                                     i * args.seq_len:(i + 1) * args.seq_len],
                              kv_cache[:, :, indices[i]],
                              atol=1e-1):
            print(f"KV cache retrieved does not match \
                    the stored one for sequence {i}")
    # -----------------------------------------------
    ...
    start_time = time.time()
    for i in range(args.seq_num):
        lmcache_retrieve_kv_hash(engine, hash_pool[i],
                                 kv_cache[:, :, indices[i]], retrieve_status)
        if torch.any(retrieve_status == RetrieveStatus.FAIL):
            print(
                f"Failure in retrieving KV cache using hash for sequence {i}")
