...
sed -i "s|blend_special_str: .*|blend_special_str: \"$BLEND_SPECIAL_STR\"|g" lmcache_blend_gpu_1.yml
...
for ratio in "${blend_recompute_ratios[@]}"; do
  ...
  sed -i "s|blend_recompute_ratios: .*|blend_recompute_ratios: ${ratio}|g" lmcache_blend_gpu_1.yml
  echo "  blend_recompute_ratio set to ${ratio}"
  
  # Ensure results directory exists
  results_dir="results_analysis/logs/${model_name}/small_dataset/use_gpu_vlcache_recompute${ratio}"
  mkdir -p "$results_dir"
