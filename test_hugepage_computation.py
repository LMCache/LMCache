...
def test_matrix_operations_with_hugepage():
    ...
    try:
        ...
        if total_elements * element_size <= size:
            ...
            print(f"2. Matrix size: {matrix_size}x{matrix_size}, total elements: {total_elements}")
            ...
            assert abs(total_sum - expected_sum) < 1e-3, f"Sum verification failed: {total_sum} vs {expected_sum}"
            ...
            print(f"   ❌ Matrix too large for allocated memory")
            lmc_ops.free_pinned_hugepage_ptr(ptr, size)
            return False
...
def test_hugepage_allocator_computation():
    ...
    try:
        # 1MB hugepage 할당자 생성
        size = 1024 * 1024
        print(f"1. Creating HugepageMemoryAllocator with {size / (1024*1024):.1f} MB...")
        
        allocator = HugepageMemoryAllocator(size)
        print(f"   ✅ Allocator created successfully")
        ...
        print("4. Cleaning up...")
        allocator.close()
        print(f"   ✅ Allocator closed successfully")
...
def test_performance_comparison():
    ...
    try:
        ...
        for size in sizes:
            ...
            print(f"    Computation speedup: {regular_compute_time/hugepage_compute_time:.2f}x")
