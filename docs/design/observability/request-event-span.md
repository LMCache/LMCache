...
computed at the emit site in `lmcache/v1/multiprocess/services/blend.py`:
...
| `lmcache/v1/multiprocess/services/blend.py` | Prefix probe in `cb_lookup_pre_computed`; lazy registration; `has_chunk` on `BlendTokenRangeMatcher`; `prefix_hits` in `CB_LOOKUP_END` metadata |
