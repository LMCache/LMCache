...
|  (no pings until traffic) |   (refresh too)   | LMCacheDriven/          BlendService |
...
then passed to `drop_instance_state(id)` on every target; `BlendService` drops
...
liveness methods (`touch`/`reap`/`count`); `BlendService` overrides only
...
`BlendService` ever implemented it.)
