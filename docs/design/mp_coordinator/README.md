...
lmcache/mp_coordinator/
  ...
  __main__.py           # uvicorn entrypoint (`python -m lmcache.mp_coordinator`)
...
wired into `lmcache/multiprocess/http_server.py`'s lifespan and configured
by a `CoordinatorConfig` (`lmcache/multiprocess/config.py`), built from
