...
Keep ``lmcache.mp.lazy_offload=false`` (the default) when using this
prefix-cache-disabled setup. Both lazy-offload policies require vLLM prefix
caching and reject this configuration at startup. In particular,
Unlimited-OCR disables vLLM prefix caching in its model configuration, so
its supported path is immediate prompt-only offload, not lazy offload.
