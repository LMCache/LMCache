...
/**
 * Calculate the bytes the staging section of one step moves. Counts all of
 * its staging copies, i.e. whole memory objects.
 */
inline int64_t calculate_staging_section_bytes(const BatchStep& step) {
  int64_t bytes = 0;
 ...
 */
inline int64_t calculate_kernel_section_bytes(
  ...
  for (const auto& step : batch_steps) {
    ...
    }
    {
      ...
      for (const auto& launch : step.launches) {
        ...
        // tensor views so we can reuse the existing kernel entry points without
        // touching any of their code. The backing storage is owned by the
        // caller's tensors (kept alive for the duration of this call); these
