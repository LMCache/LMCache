...
LocalCPUBackend (hugepage-backed pinned tensor)
                  ...
                  v
     RawBlockCore._write_spdk_buffers()   # batched across keys
                  |
                  |  pointers collected; registered buffers are
                  |  used zero-copy and headers are written into
                  |  the HeaderBufferPool DMA pool
                  v
     SpdkIoEngineFFI.batch_io_submit (op=IO_WRITE; returns (rc, batch_id))
                  |
                  |  batch submitted to the SPDK io_worker thread
                  v
     SpdkIoEngineFFI.wait_batch (calls core_spdk_wait_batch)
                  |
                  |  overall batch status (0 = success, -1 = failure)
                  v
     NVMe device (via SPDK)
```

The main data path (writes and reads) submits operations as a batch through
the public `batch_io_submit` wrapper (which forwards to `core_spdk_batch_io_submit`).
`core_spdk_batch_io_submit` returns `0` on success and `-1` on failure, with the
assigned `batch_id` written through an out-parameter; `batch_io_submit` surfaces this
as a `(rc, batch_id)` tuple. The batch is then awaited via the public
`wait_batch` wrapper (which forwards to `core_spdk_wait_batch`), returning an
overall status: `0` if every I/O succeeded, `-1` if any I/O failed. The
single-entry `spdk_write_read_external`
(`core_spdk_io`), which is reserved for metadata/checkpoint I/O, which is too
small to benefit from batching.
...
SPDK mode uses one pre-allocated DMA buffer pool to avoid per-operation allocations on the hot path:

| Pool | Purpose | Size |
|------|---------|------|
| `HeaderBufferPool` | Metadata header writes | 32 x `block_align` |

Checkpoint payload writes allocate a single `spdk_dma_zmalloc` buffer on demand and free it after the write, rather than keeping a persistent pool. Checkpoints are infrequent and serialized under the core lock, so a pinned pool would only waste hugepage memory while idle.

The `HeaderBufferPool` uses `spdk_dma_zmalloc` for cache-line-aligned, DMA-safe memory.
