...
class NativeServer {
 ...
 private:
  ...
  bool TrySendImmediateRequest(void* socket, const Frames& frames,
                               std::uint8_t request_type);
  bool TryHandleReadyLookupResult(const RequestTask& task, Frames* response);
  void TrackPendingLookup(const RequestTask& task);
  bool HasQueuedTasksLocked() const;
  std::size_t QueuedTaskCountLocked() const;
  void PushTaskLocked(RequestTask task);
  RequestTask PopTaskLocked();
  bool DrainResponses(void* socket);
  bool WaitForResponsesUntil(
      void* socket,
      std::chrono::steady_clock::time_point deadline);
  std::size_t ObserveRoutingClientCount(const Frame& identity);
  ...
  std::uint64_t FinishLookup(const std::string& request_id,
                             std::uint32_t world_size, bool has_worker_id,
  ...
  void ReleaseLocksForRequestKeys(const std::string& request_id,
                                  const std::vector<std::string>& object_keys);
  std::optional<std::uint64_t> LookupResult(const Frames& payloads,
                                            bool erase_result);
  bool HasRegisteredContext(const std::string& model_name,
  ...
  std::deque<RequestTask> control_tasks_;
  std::deque<RequestTask> retrieve_tasks_;
  std::deque<RequestTask> lookup_tasks_;
  std::deque<RequestTask> store_tasks_;
  ...
  // ZMQ-loop only; three identities means readiness plus real client fan-in.
  std::vector<Frame> routing_clients_;
  ...
  std::unordered_set<std::string> pending_lookup_results_;
