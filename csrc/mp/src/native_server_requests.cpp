...
NativeServer::Frames NativeServer::HandleRequest(const RequestTask& task) {
  ...
  switch (type) {
    ...
    case RequestType::kQueryPrefetchStatus:
      if (const auto result = LookupResult(task.payloads, true)) {
        response.push_back(msgpack::EncodeUnsigned(*result));
      } else {
        response.push_back(msgpack::EncodeNil());
      }
      break;
    case RequestType::kQueryPrefetchLookupHits:
      if (const auto result = LookupResult(task.payloads, false)) {
        response.push_back(msgpack::EncodeUnsigned(*result));
      } else {
        response.push_back(msgpack::EncodeNil());
      }
...
bool NativeServer::HasRegisteredContext(const std::string& model_name,
                                        std::uint32_t world_size) const {
  std::lock_guard<std::mutex> lock(registered_contexts_mu_);
  for (const auto& entry : registered_contexts_) {
    const RegisteredContext& context = entry.second;
    if (context.model_name == model_name && context.world_size == world_size) {
      return true;
    }
  }
  return false;
...
bool NativeServer::HasRegisteredContext(const IpcCacheEngineKey& key) const {
  return HasRegisteredContext(key.model_name, key.world_size);
}

std::uint64_t NativeServer::FinishLookup(
    const std::string& request_id, std::uint32_t world_size, bool has_worker_id,
    const std::vector<std::string>& object_keys, bool store_result) {
  ReleaseLocksForRequest(request_id);
  ...
  const std::uint64_t divisor = has_worker_id ? 1 : world_size;
  const std::uint64_t hit_chunks = divisor == 0 ? 0 : hit_objects / divisor;
  {
    std::lock_guard<std::mutex> lock(lookup_results_mu_);
    if (store_result) {
      lookup_results_[request_id] = hit_chunks;
      pending_lookup_results_.erase(request_id);
    }
    if (!locked_object_keys.empty()) {
      lookup_locks_[request_id] = std::move(locked_object_keys);
    }
  }
  return hit_chunks;
}
...
std::uint64_t NativeServer::HandleLookupPayload(const Frames& payloads,
  ...
  std::string error;
  if (RegisteredContextCount() == 0) {
    // No registered KV layout means lookup cannot hit; only request_id is
    // needed to preserve the later QUERY_PREFETCH_STATUS result.
    auto request_id = DecodeNoContextLookupRequestId(
        payloads[0].data(), payloads[0].size(), &error);
    if (!request_id) {
  ...
  if (!key) {
    ...
    auto request_id = DecodeNoContextLookupRequestId(
        payloads[0].data(), payloads[0].size(), &request_id_error);
    if (store_result && request_id) {
      std::lock_guard<std::mutex> lock(lookup_results_mu_);
      lookup_results_[*request_id] = 0;
      pending_lookup_results_.erase(*request_id);
    }
  ...
  if (!HasRegisteredContext(*key)) {
    if (store_result) {
      ...
      pending_lookup_results_.erase(key->request_id);
    }
    return 0;
  }
  ...
  return FinishLookup(key->request_id, key->world_size, key->worker_id.has_value(),
                      object_keys, store_result);
}
...
std::optional<NativeServer::KvTransferPlan> NativeServer::BuildKvTransferPlan(
  ...
  const char* request_name = retrieve ? "RETRIEVE" : "STORE";
  if (payloads.size() < expected_payloads) {
    (void)RecordInvalidPayload(std::string(request_name) +
  ...
  const auto event_handle = msgpack::DecodeBinary(payloads[3]);
  if (!token_key || !instance_id || !gpu_block_ids || !event_handle) {
    std::string message = std::string("failed to decode ") + request_name +
                          " payload metadata";
    if (!error.empty()) {
      message += ": " + error;
    }
  ...
  if (!token_key->worker_id.has_value()) {
    (void)RecordInvalidPayload(std::string(request_name) +
                               " requires worker_id");
    return std::nullopt;
  }
  ...
  if (context.model_name != token_key->model_name ||
      context.world_size != token_key->world_size) {
    (void)RecordInvalidPayload(std::string(request_name) +
  ...
  std::vector<std::string> object_keys = ObjectKeyStringsForIpcKey(
      *token_key, config_.chunk_size, token_key->start, token_key->end, &error);
  if (!error.empty()) {
    (void)RecordInvalidPayload(std::string("failed to expand ") + request_name +
                               " ObjectKeys: " + error);
    return std::nullopt;
  }
  if (object_keys.empty() && token_key->start != token_key->end) {
    (void)RecordInvalidPayload(std::string(request_name) +
  ...
  IpcCacheEngineKey plan_key = std::move(*token_key);
  return KvTransferPlan{
      .key = std::move(plan_key),
...
void NativeServer::ForceClearCache() {
  ...
  std::string cuda_clear_error;
  if (!ClearCudaTransferDeviceCache(&cuda_clear_error)) {
    std::cerr << "native CUDA device cache clear failed: " << cuda_clear_error
              << "\n";
  }
  ClearL2Adapters();
  force_clear_epoch_.fetch_add(1, std::memory_order_acq_rel);
...
std::optional<std::uint64_t> NativeServer::LookupResult(const Frames& payloads,
                                                        bool erase_result) {
  if (payloads.empty()) {
  ...
  if (it == lookup_results_.end()) {
    if (pending_lookup_results_.count(*request_id) != 0) {
      return std::nullopt;
    }
    return 0;
  }
