...
class Reader {
 public:
  ...
  bool ReadBinary(std::vector<std::uint8_t>* out) {
    if (!CanRead(1)) {
      return false;
    }
    const std::uint8_t tag = data_[pos_++];
    ...
    out->assign(data_ + pos_, data_ + pos_ + static_cast<std::size_t>(len));
    pos_ += static_cast<std::size_t>(len);
    return true;
  }
...
bool SkipTokenArray(Reader* reader) {
  std::uint64_t len = 0;
  if (!reader->ReadArrayLen(&len)) {
    return false;
  }
...
std::optional<std::string> DecodeNoContextLookupRequestId(
  ...
  for (std::uint64_t i = 0; i < map_len; ++i) {
    ...
    if (field == "model_name") {
      ...
      has_model_name = reader.ReadString(&model_name);
      if (!has_model_name) {
        SetError(error, "invalid model_name field");
        return std::nullopt;
      }
      ...
      world_size = static_cast<std::uint32_t>(value);
      has_world_size = true;
    } else if (field == "worker_id") {
      bool is_nil = false;
      if (!reader.TryReadNil(&is_nil)) {
      ...
      if (is_nil) {
        ...
        worker_id = static_cast<std::uint32_t>(value);
      }
      has_worker_id = true;
    } else if (field == "token_ids") {
      if (!SkipTokenArray(&reader)) {
        SetError(error, "invalid token_ids field");
        return std::nullopt;
      }
      has_token_ids = true;
      ...
      std::uint64_t end = 0;
      if (!reader.ReadUnsigned(&end)) {
        SetError(error, "invalid end field");
        return std::nullopt;
      }
      has_end = true;
    } else if (field == "request_id") {
      has_request_id = reader.ReadString(&request_id);
      if (!has_request_id) {
        SetError(error, "invalid request_id field");
        return std::nullopt;
      }
    } else if (field == "cache_salt") {
      if (!reader.ReadString(&cache_salt)) {
        SetError(error, "invalid cache_salt field");
        return std::nullopt;
      }
    } else if (!reader.SkipValue()) {
  ...
  if (worker_id && *worker_id >= world_size) {
    SetError(error, "worker_id must be less than world_size");
    return std::nullopt;
  }
  if (!ValidateCacheSalt(cache_salt)) {
    SetError(error, "invalid cache_salt field");
    return std::nullopt;
  }
  if (error != nullptr) {
