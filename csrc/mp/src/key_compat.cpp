...
bool rank_fields_fit_byte(std::uint32_t world_size, std::uint32_t global_rank,
                          std::uint32_t local_world_size,
                          std::uint32_t local_rank) {
  return world_size <= 255 && global_rank <= 255 && local_world_size <= 255 &&
         local_rank <= 255;
}
...
std::uint32_t lmcache_mp_cpp_compute_kv_rank(std::uint32_t world_size,
                                             ...
                                             std::uint32_t local_world_size,
                                             std::uint32_t local_rank) {
  if (!rank_fields_fit_byte(world_size, global_rank, local_world_size,
                            local_rank)) {
...
int lmcache_mp_cpp_expand_kv_ranks(std::uint32_t world_size,
  ...
  if (rank >= world_size || rank > 255 || max_ranks < 1) {
    return -2;
  }
  out_ranks[0] =
      lmcache_mp_cpp_compute_kv_rank(world_size, rank, world_size, rank);
