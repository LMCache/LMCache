...
std::uint32_t ParsePositiveU32(const std::string& value,
                               const char* name) {
...
void ApplyEngineConfigValues(
  ...
  if (const auto it = values.find("chunk_size"); it != values.end()) {
    config->chunk_size = ParsePositiveU32(it->second, "chunk_size");
  }
  ...
  try {
    ...
    for (int i = 1; i < argc; ++i) {
      ...
      if (arg == "--host") {
        ...
        config.chunk_size = ParsePositiveU32(
            NextArg(argc, argv, &i, arg), "--chunk-size");
