...
// Shape conventions below: HS is the logical per-head width of one K or V
// plane; CS = 2 * HS is the packed per-head content width and is what the
// fused entry points' head_size parameter carries.
enum class MemObjKVLayout : int {
  UNSPECIFIED = 0,
  // True split-KV object: [2, num_layers, num_tokens, num_heads * HS].
  SPLIT_KV_2LTD = 1,
  // Packed single plane mirroring the engine's fused rows:
  // [1, num_layers, num_tokens, num_heads * CS].
  FUSED_PACKED = 2,
};
