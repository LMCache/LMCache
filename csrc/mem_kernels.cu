...
// For vLLM
__device__ __forceinline__ int64_t page_buffer_offset_vllm(
    const int k_or_v, const int head_idx, const int64_t cache_block_id,
    const int scalar_offset,
    const int page_buffer_size, const int num_heads,
    const int scalars_per_head_in_gpu_kv) {
  return (int64_t)k_or_v * page_buffer_size * scalars_per_head_in_gpu_kv  +
         (int64_t)cache_block_id * num_heads * scalars_per_head_in_gpu_kv +
         (int64_t)head_idx * scalars_per_head_in_gpu_kv +
         scalar_offset;
}
...
key_value_offset_vllm(const int k_or_v, const int layer_idx, const int head_id,
                      const int scalar_offset,
                      const int num_layers, const int num_heads,
  ...
  return (int64_t)k_or_v * num_layers * num_heads * scalars_per_head +
         ...
         (int64_t)head_id * scalars_per_head +
         scalar_offset;
}
...
__global__ void load_and_reshape_multi_layer_vllm_kernel(
    scalar_t* __restrict__ key_value,           // [2, num_layer, num_heads,
                                                // scalars_per_head]
    scalar_t** __restrict__ paged_buffer_ptrs,  // [num_layers] * [2, num_blocks, block_size, num_heads, head_size]
    const int64_t* __restrict__ slot_mapping,   // [num_tokens]
    const int scalars_per_head,
    const int scalars_per_head_in_gpu_kv,
    const int elements_per_xword,
  ...
  for (int i = tid; i < scalars_per_head; i += num_threads) {
    ...
    const int64_t cache_block_id = slot_idx / block_size;

    const int64_t lmcache_offset = key_value_offset_vllm(
        k_or_v, layer_id, head_id,
        i,
        num_layers, num_heads,
        scalars_per_head);

    const int64_t vllm_offset = page_buffer_offset_vllm(
        k_or_v, head_id, cache_block_id,
        i,
        page_buffer_size, num_heads,
        scalars_per_head_in_gpu_kv);
 ...
 */
template <typename T>
void multi_layer_kv_transfer_vllm_templated(
    torch::Tensor&
  ...
  int num_heads = key_value.size(2);
  ...
  int num_xwords_in_gpu_kv = block_size * head_size / elements_per_xword;
  ...
  if (not direction) {
    lmc::load_and_reshape_multi_layer_vllm_kernel<T, false>
    ...
    lmc::load_and_reshape_multi_layer_vllm_kernel<T, true>
        <<<grid, block, 0, stream>>>(key_value_ptr, page_buffer_ptrs, slot_mapping_ptr,
                                     num_xwords, num_xwords_in_gpu_kv, elements_per_xword,
<<<<<<< HEAD
                                     page_buffer_size, num_layers, num_heads, block_size, head_size);
...
void multi_layer_kv_transfer_vllm(torch::Tensor& key_value,
  ...
  #define LAUNCH_MULTI_LAYER_KV_TRANSFER(type)      \
    do {                                            \
      multi_layer_kv_transfer_vllm_templated<type>( \
