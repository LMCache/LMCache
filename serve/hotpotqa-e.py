...
FILES = [
    'dataset/hotpotqa-e_processed_v2.csv',
]
...
def execute_openai_request_with_output(row, model: str, client: openai.Client) -> Tuple[float, float, float, float, str]:
    ...
    prompt += f"""
    You are a world‑class multi‑hop question‐answering assistant.  
    The supporting paragraphs are given below:

    {row.context}

    The question is given below. Please supply only the correct Answer within 5 words.
