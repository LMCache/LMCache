...
def main():
    parser = argparse.ArgumentParser(
        description="""
        将输入 CSV 中的所有记录循环两遍输出，第一遍 occurrence_number=1，第二遍 occurrence_number=2。
        数据集长度变为原来的两倍。
    ...
    parser.add_argument(
        ...
        help="输入 CSV 文件路径 (包含 title 和约若干行数据)。"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="随机数种子（可选），仅影响可复现性，但本脚本不使用随机性。"
    )
